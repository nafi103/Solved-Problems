...
bool check(int mid,vpi &v){
    int taken = 0;
    for(int i = v.size()-1;i>=0; i--){
        if(v[i].ff/mid==0) break;
        taken+=v[i].ff/mid;
    }
    return taken>=k;
}

int bs(int l,int r, vpi &v){
    if(l>r) return r;
    int mid = (l+r)/2;
    if(check(mid,v)) return bs(mid+1,r,v);
    else return bs(l,mid-1,v);
}
...
void solve()
{
    ...
    sort(all(ans));
    int findMax = bs(1,max(1ll,n-1),ans), i = ans.size()-1;
    while(k){
        int take = min(ans[i].ff/findMax,k);
        k-=take;
        while(take--) final.push_back(ans[i].ss); 
        i--;
    }
    writev(final);
}
