...
int allSame(char x, int l, int r){
    int cnt = 0;
    for(int i = l-1; i<r; i++){
        if(str[i]!=x) cnt++;
    }
...
int makeCgood(char a, int l, int r){
    if(l==r){
        if(str[l-1]==a) return 0;
        else return 1;
    }
    int mid = l+(r-l+1)/2 - 1;
    return min(allSame(a,l,mid)+makeCgood(a+1,mid+1,r),allSame(a,mid+1,r)+makeCgood(a+1,l,mid));
}
