...
void findParent(int node, int parent, vi &parents,vi &level){
    level[node] = level[parent]+1;
    parents[node] = parent;
    for(auto &x: t[node]){
        if(x!=parent){
            findParent(x,node,parents,level);
        }
    }
}
...
void solve()
{
    ...
    level[0] = -1;
    findParent(1,0,parent,level);
    set<pii>s;
    for(int i = 2; i<=n; i++){
        s.insert({-level[i],i});
    }
    while(s.size()){
        if(s.begin()->ff>=-2) break;
        int currNode = parent[s.begin()->ss];
        ans++;
        auto it = s.find({-level[currNode],currNode});
        if(it!=s.end()) s.erase(it);
        for(auto &x: t[currNode]){
            auto it = s.find({-level[x],x});
            if(it!=s.end()) s.erase(it);
        }
    }
    cout<<ans<<endl;
