...
int check(int skip, int n, vpi &v){
...
void solve()
{
    ...
    int currL = (*l.rbegin()).ff,currR = (*r.begin()).ff;
    int ans = max(0ll,currR - currL);
    if(l[currL].size()==1){
        ans = max(ans,check(l[currL][0],n,v));
    }
    if(r[currR].size()==1){
        ans= max(ans,check(r[currR][0],n,v));
    }
    cout<<ans<<endl;
}
