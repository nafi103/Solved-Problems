...
int findAns(vector<int>&v, int n, int k){
    int ans = 0;
    for(int i = 0; i<n; i++){
        int size = log10(v[i])+1,modVal = v[i]%k;
        if(modVal) ans+=(mp[size][k-modVal]);
        else ans+=(mp[size][modVal]);
        if(modVal==0 or k-modVal==(v[i]*power[size])%k) ans--;
    }
    return ans;
}
