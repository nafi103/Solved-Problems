...
void solve()
{
    ...
    vector<vi>dp(n,vi(n+100,INT_MAX));
    dp[n-1][0] = 0;
    for(int i = n-2; i>=0; i--){
        dp[i][0] = 0;
        for(int j = 1; j<n+100; j++){
            int banacchi = v[i]+j;
            debug(banacchi)
            int porerta = banacchi - 1;
            int barateHobe = porerta - v[i+1];
            if(barateHobe>=n+100) continue;
            debug(barateHobe)
            if(barateHobe<=0) dp[i][j] = j;
            else dp[i][j] = dp[i+1][barateHobe]+1+max(porerta-v[i],0ll);
            if(dp[i][j]<=k) ans = max(ans,v[i]+j);
        }
    }
    debug(dp)
    cout<<ans<<endl;
