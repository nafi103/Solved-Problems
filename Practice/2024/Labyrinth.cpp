...
void solve()
{
    ...
    vector<vpi>parent(n,vpi(m,{inf,inf}));
    ...
    while(!q.empty()){
        ...
        if(v[i][j]=='B') break;
        rep(k,0,4){
            if(i+x[k]>=0 and i+x[k]<n and j+y[k]>=0 and j+y[k]<m and v[i+x[k]][j+y[k]]!='#' and !visited[i+x[k]][j+y[k]]){
    ...
    if(visited[desi][desj]) {
        ...
        while(true){
            cnt++;
            int pari = parent[desi][desj].ff, parj = parent[desi][desj].ss;
            ans.pb(dirr(pari-desi,parj-desj));
            desi = pari;
            desj= parj;
            if(desi==srci and desj==srcj) break;
        }
        cout<<cnt<<endl;
        reverse(all(ans));
        cout<<ans<<endl;
