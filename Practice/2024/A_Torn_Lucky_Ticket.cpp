...
void solve()
{
    ...
    vi v(n);
    readv(v);
    vector<vi>dp(6,vi(46,0));
    for(auto &x: v){
        int cnt = log10(x)+1,digSum = f(x);
        dp[cnt][digSum]++;
    }
    for(auto x: v){
        int mult = 1,xSum = f(x), size = log10(x)+1;
        for(int i = size; i>=1; i--,mult*=10){
            int leftPart = x/mult,rightPart = x%mult;
            int dSumL = f(leftPart), dSumR = f(rightPart);
            int lSz = log10(leftPart)+1, rSz = rightPart? log10(rightPart)+1: 0;
            debug(lSz) debug(rSz)
            int needSz, needSum;
            if(lSz>rSz){
                needSz = lSz - rSz;
                needSum = f(leftPart) - f(rightPart);
            }else{
                needSz = rSz - lSz;
                needSum = f(rightPart) - f(leftPart);
            }
            debug(leftPart) debug(rightPart) debug(needSz) debug(needSum)
            if(needSum<0) continue;
            ans+=dp[needSz][needSum];
        }
    }
    cout<<ans<<endl;
}
