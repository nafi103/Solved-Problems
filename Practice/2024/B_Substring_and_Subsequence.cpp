...
void solve()
{
    ...
    int mx = 0;
    for(int j = 0; j<m; j++){
        if(mp.find(b[j])==mp.end()) continue;
        int last = mp[b[j]][0],cnt = 1;
        for(int k = j+1; k<m; k++){
            if(mp.find(b[k])==mp.end()) break;
            int next = upper_bound(all(mp[b[k]]),last) - mp[b[k]].begin();
            if(next == mp[b[k]].size()) break;
            cnt++;
            last = mp[b[k]][next];
        }
        mx = max(mx,cnt);
