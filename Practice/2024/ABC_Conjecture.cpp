...
void solve()
{
    ...
    for(int i = 0; i<n; i++){
        if(a[i]=='b') bs.pb(i);
        if(a[i]=='c' and b[i]=='a') cs.pb(i);
    }
    for(int i = 0; i<n; i++){
        if(a[i]!=b[i]){
            if(a[i]=='b' or a[i]=='c' or b[i]=='b' or bs.empty() or cs.empty()){
                cout<<"No"<<endl;
                return;
            }
            int nextB = upper_bound(all(bs),i) - bs.begin();
            if(nextB==b.size()){
                cout<<"No"<<endl;
                return;
            }
            int nextC = upper_bound(all(cs),bs[nextB]) - cs.begin();
            if(nextC==cs.size()){
                cout<<"No"<<endl;
                return;
            }
