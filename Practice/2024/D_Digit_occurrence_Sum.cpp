...
template <class T> using pbds = tree<T, null_type, greater<T>, rb_tree_tag, tree_order_statistics_node_update >;
...
/****************************************************************/
void addDig(int num, map<int,int>&mp){
    while(num>0){
        int rem = num%10;
        mp[rem]++;
        num/=10;
...
void eraseDig(int num, map<int,int>&mp){
    while(num>0){
        int rem = num%10;
        mp[rem]--;
        num/=10;
...
void showAns(int num, map<int,int>&mp){
    int ans = 0;
    while (num > 0)
    {
        int rem = num % 10;
...
void solve()
{
    ...
    while(op--){
        ...
        cin>>c>>val;
        if(c=='+'){
            int pos = d.order_of_key(val);
            if(pos<d.size() and *d.find_by_order(pos)==val){
                eraseDig(val,mp);
                d.erase(val);
            }else{
                d.insert(val);
            ...
            if(val>d.size()) continue;
            else{
                eraseDig(*d.find_by_order(val-1),mp);
                d.erase(d.find_by_order(val-1));
            }
        }else{
            int pos = d.order_of_key(val);
            if(pos<d.size() and *d.find_by_order(pos)==val){
