...
void solve()
{
    ...
    int msbofA = log2(a),msbofB = log2(b);
    int p = (1<<msbofA);
    while(!((b&p)==0 and (a&p)!=0)){
        p>>=1;
    }
    int partofA = (((p<<1))-1)&a, partofB = ((p<<1)-1)&b, possibleAns,anotherPossible,third = INT_MAX;
    possibleAns = partofA - partofB;
    if(b>a) possibleAns++;
    if(msbofA>=msbofB) anotherPossible = INT_MAX;
    else{
        msbofA = (1<<(msbofA+1));
        while((msbofA&b)==0) msbofA<<=1;
        anotherPossible = msbofA - a;
        if(msbofA!=b) anotherPossible++;
    }
    if(a<=b){
        for(int i = a; i<=b; i++){
            if((i|b) == b){
    ...
    cout<<min({possibleAns,anotherPossible,third})<<endl;
}
