...
/****************************************************************/
vector<int>in_cycle,visited, in_path;

void dfs(int node, vector<vi>&g){
    visited[node] = true;
    in_path[node] = true;
    for(auto &x: g[node]){
        if(!visited[x]){
            dfs(x,g);
            in_cycle[node] |= in_cycle[x];
        }else if(in_path[x] or in_cycle[x]){
            in_cycle[node] = true;
        }
    }
    in_path[node] = false;
...
void solve()
{
    ...
    int sz = mp.size();
    g.resize(sz);
    in_cycle.assign(sz, false);
    visited.assign(sz, false);
    in_path.assign(sz, false);
    for(auto &x: nodes){
        if(!visited[x]){
            dfs(x,g);
        }
    ...
    while(cin>>str){
        int node = mp[str];
        cout<<str<<" "<<(in_cycle[node]?"safe":"trapped")<<endl;
    }
}
