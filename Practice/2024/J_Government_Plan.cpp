...
void dfs(int node, vector<bool>&visited,vector<vi>&g,int &num, vi &graphNo){
    visited[node] = true;
    graphNo[node] = num;
    for(auto &x: g[node]){
        if(!visited[x]) dfs(x,visited,g,num,graphNo);
    }
}

void tour(int node, vector<bool>&visited,vector<vi>&g){
    visited[node] = true;
    for(auto &x: g[node]){
        if(!visited[x]) tour(x,visited,g);
    }
}
...
void solve()
{
    ...
    cin>>n>>m>>a;
    vector<vi>g(n+1);
    vector<bool>visited(n+1,false),airport(n+1,false);
    vector<vi>edges;
    while(m--){
        vi tmp(3);
        readv(tmp);
    ...
    vector<int>graphNo(n+1,-1);
    vector<bool>seen(n+1,false);
    int num = 0;
    for(int i = 1; i<=n; i++){
        if(!seen[i]){
            dfs(i,seen,g,num,graphNo);
            num++;
        }
    }
    seen.assign(n+1,false);
    for(auto &x: edges){
        if(graphNo[x[0]]!=graphNo[x[1]]){
            int u = x[0],v = x[1],wt = x[2];
            if(wt<a){
                cost+=wt;
                num = graphNo[u];
                dfs(v,seen,g,num,graphNo);
                g[u].pb(v);
                g[v].pb(u);
            }else{
                break;
