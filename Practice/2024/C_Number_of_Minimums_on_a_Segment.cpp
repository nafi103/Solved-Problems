...
void buildSegTree(int node, int b, int e){
    ...
    t[node].first = min(t[left].first,t[right].first);
    t[node].second = 0;
    if(t[node].first == t[left].first)  t[node].second+=t[left].second;
    if(t[node].first == t[right].first)  t[node].second+=t[right].second;
}

pair<int,int> query(int node, int b, int e, int l, int r){
    if(e<l or b>r)  return {INT_MAX, 0};
    if(b>=l and e<=r)   return t[node];
    int left = 2*node, right = 2*node+1, mid = (b+e)/2;
    pair<int,int> qLeft, qright;
    qLeft = query(left,b,mid,l,r);
    qright = query(right,mid+1, e, l,r);
    if(qLeft.first==qright.first){
        return {qLeft.first,qLeft.second+qright.second};
    }else{
...
void update(int node, int b, int e, int i, int x){
    ...
    t[node].first = min(t[left].first, t[right].first);
    t[node].second = 0;
    if(t[node].first == t[left].first)
        t[node].second += t[left].second;
    if(t[node].first == t[right].first)
...
void solve()
{
    ...
    v.resize(n+1);
    t.resize(4*n+10);
    rep(i,0,4*n+10)    t[i].first = INT_MAX;
    rep(i,1,n+1)    cin>>v[i];
    debug(v)
    buildSegTree(1,1,n);
    debug(t)
