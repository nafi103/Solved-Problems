...
void solve()
{
    ...
    for(int i = 0; i<n; i++){
        ...
        if(t[i].currColor!=last){
            initialK++;
            last = t[i].currColor;
        }
    }
    if(!zeroPresent and initialK>=K){
        cout<<((initialK==K)? 0:-1)<<endl;
        return;
    }
    vector<vector<vi>> dp(n,vector<vi>(m+1,vi(K+1,INT_MAX)));
    if(t[0].currColor==0){
        for(int j = 1; j<=m; j++) dp[0][j][1] = t[0].paintCost[j-1];
    }else{
        dp[0][t[0].currColor][1] = 0;
    }
    for(int i = 1; i<n; i++){
        if(t[i].currColor!=0){
            for(int k = 1; k<=K; k++){
                int mn = dp[i-1][t[i].currColor][k];
                if(k>1){
                    for(int j = 1; j<=m; j++){
                        if(x!=j) mn = min(mn, dp[i-1][j][k-1]);
                    }
                }
                dp[i][t[i].currColor][k] = mn;
            }
        }else{
            for(int j = 1; j<=m; j++){
                for(int k = 1; k<=K; k++){
