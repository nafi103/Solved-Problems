...
void printAns(int n){
    if(store[n][0].ff==0 and store[n][0].ss==0){
        cout<<"There are no solutions for "<<n<<"."<<endl;
        return;
    }
    for(auto &[f,s]:store[n]){
        cout<<num(f)<<" / "<<num(s)<<" = "<<n<<endl;
    }
}

void solve(int n)
{
    if(store[n].size()) {printAns(n);return;}
    for(int i = 1234; i<=98765/n; i++){
        int mask = (i<10000), tmp;
        tmp = i;
        while(tmp>0){
            mask|=(1<<(tmp%10));
            tmp/=10;
        ...
        if(mask==(1<<10)-1) store[n].pb({n*i,i});
    }
    if(store[n].empty()){
        store[n].pb({0,0});
    }
