...
int32_t main()
{
    ...
    for(int i = 0; i<mx; i++){
        for(int j = 1; j<n; j++){
            if(i<(int)v[j].size() and i<(int)v[j-1].size()){
                if(v[j][i]!=v[j-1][i] and v[j-1].substr(0,i)==v[j].substr(0,i)){
                    int V = v[j][i] - 'A', U = v[j-1][i] - 'A';
                    g[U].pb(V);
                }
            }
        }
    ...
    for(int i = 0; i<26; i++){
        for(auto &x: g[i]){
            if(in_degree[i]==-1) in_degree[i] = 0;
            if(in_degree[x]==-1) in_degree[x] = 0;
            in_degree[x]++;
        }
    }
    queue<int>q;
    ...
    while(!q.empty()){
        ...
        for(auto &x: g[curr]){
            in_degree[x]--;
            if(in_degree[x]==0 and !visited[x]) q.push(x);
        }
    }
    cout<<endl;
}