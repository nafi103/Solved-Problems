...
int w,h;
Direction _up(0,-1),_down(0,1),_left(-1,0),_right(1,0);

Direction initial_direction(int i, int j){
    if(i==0) return _down;
    else if(i==h-1) return _up;
    else if(j==0) return _right;
    return _left;
}

Direction change_direction(char mirror, Direction &curr_dir){
    if(mirror=='/'){
        if(curr_dir==_right) return _up;
        else if(curr_dir==_left) return _down;
        else if(curr_dir==_up) return _right;
        else return _left; 
    }
    if(curr_dir==_right) return _down;
    else if(curr_dir==_left) return _up;
    else if(curr_dir==_up) return _left;
    else  return _right;
}
...
void solve(int w, int h){
    ...
    for(int i = 0; i<h; i++){
        for(int j = 0; j<w; j++){
            cin>>grid[i][j];
            if(grid[i][j]=='*'){
                curr_dir = initial_direction(i,j);
                ini = i, inj = j;
            }
        }
    }
    int curri = ini+curr_dir.y, currj = inj+curr_dir.x;
    while(grid[curri][currj]!='x'){
        if(grid[curri][currj]=='\\' or grid[curri][currj]=='/'){
            curr_dir = change_direction(grid[curri][currj],curr_dir);
        }
        curri+=curr_dir.y;
        currj+=curr_dir.x;
    }
