...
int f(int l, int r){
    ...
    int leftChosen = v[l] + min(f(l+2,r),f(l+1,r-1));
    int rightChosen = v[r] + min(f(l,r-2),f(l+1,r-1));
    return dp[l][r] = max(leftChosen,rightChosen);
}
...
void solve()
{
    ...
    for(int k = 2; k<=n; k++){
        int l = 0, r = k-1;
        while(r<n){
            int l2r = (l+2>r)? 0:dp[l+2][r];
            int l1r1 = (l+1>r-1)? 0:dp[l+1][r-1];
            int lr2 = (l>r-2)? 0:dp[l][r-2];
            dp[l][r] = max(v[l]+min(l2r,l1r1), v[r]+min(lr2,l1r1));
            l++;
            r++;
        }
    }
