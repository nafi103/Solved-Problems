...
int f(int a, int b){
    ...
    int lengthWise = INT_MAX, widthWise = INT_MAX;
    for(int i = 1; i<a; i++){
        lengthWise = min(lengthWise, 1+(f(i,b)+f(a-i,b)));
    }
    for(int j = 1; j<b; j++){
        widthWise = min(widthWise,1+f(a,j)+f(a,b-j));
    }
    dp[a][b] = min(lengthWise,widthWise);
    return dp[a][b];
}
...
void solve()
{
    ...
    for(int i = 2; i<=a; i++){
        for(int j = 2; j<=b; j++){
            ...
            int lengthWise = INT_MAX, widthWise = INT_MAX;
            for(int k = 1; k<i; k++){
                lengthWise = min(1+dp[k][j]+dp[i-k][j],lengthWise);
            }
            for(int k = 1; k<j; k++){
                widthWise = min(1+dp[i][k]+dp[i][j-k],widthWise);
            }
            dp[i][j] = min(lengthWise,widthWise);
        }
    }
    cout<<dp[a][b]<<endl;
}
