...
struct Graph{
    ...
    int dijkstra(){
        ...
        vector<vi> distance(n+1, vi(n+1,MX));
        distance[1][1] = 0;
        priority_queue<tt,vector<tt>,greater<tt>>pq;
        pq.push({0,1,1});
        while(!pq.empty()){
            int c = get<1>(pq.top()), b = get<2>(pq.top()),d = get<0>(pq.top());
            pq.pop();
            if(visited[c][b] or distance[c][b]==MX) continue;
            visited[c][b] = true;
            for(auto [nbr,wt]: v[c]){
                int pBike = (s[b]>s[nbr] ? nbr:b);
                if(distance[nbr][pBike]> distance[c][b] + s[b]*wt){
                    distance[nbr][pBike] = distance[c][b] + s[b] *wt;
                    pq.push({distance[nbr][pBike], nbr, pBike});
                }
            }
        }
        int mn = MX;
        for(auto x: distance[n]) mn = min(mn,x);
        return mn;
    }
};
