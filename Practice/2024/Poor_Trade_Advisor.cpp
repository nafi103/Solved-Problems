...
struct Graph{
    ...
    int dfs(int node, vector<bool>&visited, int &mx){
        visited[node] = true;
        int ans = 1;
        bool flag = true;
        for(auto &[nbr,wt]: v[node]){
            if(!visited[nbr] and wt==mx){
                ans+=dfs(nbr,visited,mx);
            }
        }
        return ans;
    }
...
int dfs(int node, vector<bool>visited, Graph &g, int &mx){
}
