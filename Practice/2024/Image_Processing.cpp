...
void solve()
{
    int h,w,n,m;
    cin>>h>>w>>n>>m;
    int ar = h-n+1,ac = w-m+1;
    vector<vi>image(h,vi(w)),to_flip_kernel(n,vi(m)), kernel(n,vi(m)),ans(ar,vi(ac,0));
    for(auto &v: image) readv(v);
    for(auto &v: to_flip_kernel) readv(v);
    for(int i = 0, ri = n-1; i<n; i++,ri--){
        for(int j = 0, rj = m-1; j<m; j++,rj--){
            kernel[ri][rj] = to_flip_kernel[i][j];
        }
    }
    for(int i = 0; i<ar; i++){
        for(int j = 0; j<ac; j++){
            for(int k = 0; k<n; k++){
                for(int l = 0; l<m; l++){
                    ans[i][j]+=(kernel[k][l]*image[i+k][j+l]);
                }
            }
        }
    }
    for(int i = 0; i<ar; i++){
        for(int j = 0; j<ac; j++) cout<<ans[i][j]<<" ";
        cout<<endl;
    }
}
