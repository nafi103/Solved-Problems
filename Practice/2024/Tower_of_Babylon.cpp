...
int f(int prevL, int prevW){
    if(dp.find({prevL,prevW})!=dp.end()) return dp[{prevL,prevW}];
    dp[{prevL,prevW}] = 0;
    for(int j = 0; j<n; j++){
        if(v[j][0]<prevL and v[j][1]<prevW){
            dp[{prevL,prevW}] = max(dp[{prevL,prevW}], v[j][2] + f(v[j][0],v[j][1]));
        }
    }
    return dp[{prevL,prevW}];
}
