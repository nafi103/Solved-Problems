...
int f(int currTime,int pos, int cnt){
    if(pos==n) return cnt;
    int nTime1 = currTime+v[pos];
    int nTime2 = nTime1-1;
    nTime1%=h;
    nTime2%=h;
    debug(nTime1) debug(nTime2)
    return max(f(nTime1,pos+1,cnt+(nTime1>=l and nTime1<=r)),f(nTime2,pos+1,cnt+(nTime2>=l and nTime2<=r)));
}
...
void solve()
{
    ...
    dp.resize(n+1,vi(h,-1));
    dp[0][0] = 0;
    for(int i = 1; i<=n; i++){
        for(int j=0; j<h; j++){
            int prevT1 = (j-v[i] + h)%h, prevT2 = (j-(v[i]-1) + h)%h;
            debug(j);
            debug(prevT1) debug(prevT2);
            if(dp[i-1][prevT1]==-1 and dp[i-1][prevT2]==-1) continue;
            dp[i][j] = max((dp[i-1][prevT1]==-1)? INT_MIN:dp[i-1][prevT1], dp[i-1][prevT2]==-1 ? INT_MIN:dp[i-1][prevT2]) + (j>=l and j<=r);
        }
    }
    for(auto x: dp[n]){
        if(x!=-1){
