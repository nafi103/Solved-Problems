...
void solve()
{
    int n;
    cin>>n;
    vector<string>name(n),ans(n,"$");
    readv(name);
    for(int i = 0; i<n; i++){
        if(name[i]!="?") ans[i] = name[i];
    }
    int first = INT_MAX, last = INT_MIN;
    int l = 0, r = 0;
    while(r<n){
        while(l<n and name[l]=="?") l++;
        r = l+1;
        while(r<n and name[r]=="?") r++;
        if(r<n and r-l>1){
            string leftName = name[l], rightName = name[r];
            int tl = l+1, tr = r-1;
            while(tl<=tr){
                if(tl==tr){
                    ans[tl] = "middle of ";
                    ...
                    ans[tl] = "right of " + ans[tl-1];
                    ans[tr] = "left of "+ans[tr+1];
                }
                tl++;
                tr--;
            }
