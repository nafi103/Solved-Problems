...
int query(int node,int b, int e, int l, int r){
    ...
    int leftNode = 2*node, rightNode = 2*node + 1, mid = (b+e)/2;
    return query(leftNode,b,mid,l,r)+query(rightNode, mid+1, e, l,r);
}
...
void solve()
{
    ...
    t.resize(4*comp.size(),0);
    for(auto x: v){
        int calc = query(1, 1, comp.size(), 1, comp[x] - 1);
        dp[comp[x]] += (calc + 1);
        update(1, 1, comp.size(), comp[x], dp[comp[x]]);
    }
    for(auto [f,s]: dp){
        ans+=s;
    }
