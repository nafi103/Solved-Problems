...
/****************************************************************/
vector<int>dp,health;
int final,n;
vector<string>grid;

int f(int mask, vi damage){
    if(mask==final) return 0ll;
    if(dp[mask]!=-1) return dp[mask];
    int ans=INT_MAX;
    for(int i = 0; i<n; i++){
        if((mask&(1<<i))==0){
            vi tmp(n);
            int tmask = mask|(1<<i),add = (health[i]+damage[i]-1)/damage[i];
            for(int j = 0; j<n; j++){
                tmp[j] = max(damage[j],(int)(grid[i][j] - '0'));
            }
            ans = min(ans, add+f(tmask,tmp));
        }
    }
    return dp[mask] = ans;
}
...
void solve()
{
    ...
    dp.assign(final+1,-1);
    vi damage(n);
    for(int i = 0; i<n; i++) damage[i] = 1;
    cout<<f(0,damage)<<endl;
}
