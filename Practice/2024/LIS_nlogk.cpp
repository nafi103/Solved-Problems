...
void printLIS(int i, vi &parent,vi &v){
    if(parent[i]==-1){
        cout<<v[i]<<" ";
        return;
    }
    printLIS(parent[i],parent,v);
    cout<<v[i]<<" ";
}
...
void solve()
{
    ...
    vi v(n);
    readv(v);
    vector<int>parent(n,-1),LIS(n,0),id(n,0);
    int k = 0;
    for(int i = 0; i<n; i++){
        int pos = lower_bound(LIS.begin(),LIS.begin()+k,v[i]) - LIS.begin();
        LIS[pos] = v[i];
        id[pos] = i;
        if(pos) parent[i] = id[pos-1];
        if(pos==k){
            k++;
            LIS.pb(v[i]);
        }else{
    ...
    debug(k) debug(parent)
    printLIS(id[k-1],parent,v);
    cout<<endl;
}
