...
void solve()
{
    ...
    vector<int> dp(k+1,0);
    for(int i = 0; i<n; i++){
        for (int j = k - opNeeded[a[i]]; j >= 0; j--){
            int p = j+opNeeded[a[i]];
            dp[p] = max(dp[p], dp[j] + b[i]);
            ans = max(ans, dp[p]);
        }
    }
    cout<<ans<<endl;
...
int32_t main()
{
    ...
    for(int i= 2; i<N; i++){
        for(int j = 1; j<= i/2 + 1; j++){
            int next = i+i/j;
            if(next<N) opNeeded[next] = min(opNeeded[next],opNeeded[i]+1);
        }
    }
    int t = 1;
    cin >> t;
