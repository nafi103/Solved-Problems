...
int32_t main()
{
    ...
    cout.setf(ios::fixed);
    int n,m,sj;
    while(cin>>n>>m>>sj and n){
        int si = 1;
        vector<vc>grid(n+1,vc(m+1));
        for(int i = 1; i<=n; i++){
            for(int j = 1; j<=m; j++){
                cin>>grid[i][j];
            }
        ...
        bool loop = false;
        vector<vi>dfs_cnt(n+1,vi(m+1,-1));
        int dfs = 1;
        while(valid(si,sj,n,m)){
            if(dfs_cnt[si][sj]!=-1){
                loop = true;
                break;
            }
            dfs_cnt[si][sj] = dfs;
            pii add = Move[grid[si][sj]];
            si+=add.ff;
            sj+=add.ss;
            dfs++;
        }
        dfs--;
        if(loop){
            cout<<dfs_cnt[si][sj]-1<<" step(s) before a loop of "<<dfs-dfs_cnt[si][sj]+1<<" step(s)"<<endl;
        }else{
            cout<<dfs<<" step(s) to exit"<<endl;
        }
    }
}