...
void solve()
{
    ...
    for(int z = 1; z<=a; z++){
        for(int x = 1; x<=b; x++){
            for(int y = 1; y<=c; y++){
                cin>>heap[z][x][y];
                mx = max(mx,heap[z][x][y]);
                heap[z][x][y]+=(heap[z][x-1][y]+heap[z][x][y-1]-heap[z][x-1][y-1]);
                heap[z][x][y]+=(heap[z-1][x][y] - heap[z-1][x-1][y] - heap[z-1][x][y-1]+ heap[z-1][x-1][y-1]);
            }
        }
    }
                ...
                for(int x2= x1; x2<=b; x2++){
                    for(int y1 = 1;y1<=c; y1++){
                        for(int y2 = y1; y2<=c; y2++){
                            int sum = heap[z2][x2][y2]  - heap[z2][x1-1][y2]
                                    - heap[z2][x2][y1-1] + heap[z2][x1-1][y1-1]
                                    - heap[z1-1][x2][y2] + heap[z1-1][x1-1][y2]
                                    + heap[z1-1][x2][y1-1] - heap[z1-1][x1-1][y1-1];
                            mx = max(mx,sum);
                        }
                    }
                }
