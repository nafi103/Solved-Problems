...
#define yes cout<<"Find"<<endl
#define no cout<<"NotFind"<<endl
...
vector<vi>t,parents;
vector<int>level,sTree;
int mx;

void dfs(int node, int parent, int cLevel,int &time){
    inTime[node] = time++;
    level[node] = cLevel;
    parents[0][node] = parent;
    for(auto &x: t[node]){
        if(x!=parent) dfs(x,node, cLevel+1,time);
    }
    outTime[node] = time++;
}
...
int Query(int node, int b, int e, int l, int r){
    ...
    if(b>=l and e<=r)   return sTree[node];
    int left = 2*node, right = 2*node+1, mid = (b+e)/2;
...
void solve(int n)
{
    ...
    for(int i = 1; i<=n; i++){
        ...
        for(auto &x: mp[i]){
            update(1,1,time,inTime[x],1);
            update(1,1,time,outTime[x],-1);
        }
        for(auto &x: query[i]){
            int start = x[0], end = x[1], ansPos = x[2];
            int LCA = lca(start,end);
            int cnt = Query(1,1,time,inTime[LCA],inTime[start]);
            cnt+=Query(1,1,time,inTime[LCA],inTime[end]);
            cnt-=Query(1,1,time,inTime[LCA],inTime[LCA]);
            if(cnt) ans[ansPos] = true;
        }
        for(auto &x: mp[i]){
            update(1,1,time,inTime[x],0);
            update(1,1,time,outTime[x],0);
        }
    }
    for(int i = 0; i<q; i++){
        if(ans[i]) yes;
