...
void solve()
{
    int n,l,r,cost,mn = INT_MAX, mx = INT_MIN,mnCost = INT_MAX, mxCost = INT_MAX,prevCost = LLONG_MAX;
    cin>>n;
    for(int i = 0; i<n; i++){
        cin>>l>>r>>cost;
        if(l<=mn){
            if(l==mn) mnCost = min(mnCost,cost);
            else{
                prevCost = LLONG_MAX;
                mn = l;
                mnCost = cost;
        ...
        if(r>=mx){
            if(r==mx) mxCost = min(mxCost,cost);
            else{
                prevCost= LLONG_MAX;
                mx = r;
                mxCost = cost;
        ...
        if(l==mn and r==mx) prevCost = (min(prevCost,cost));
        cout<<min(mnCost+mxCost,prevCost)<<endl;
    }
}
