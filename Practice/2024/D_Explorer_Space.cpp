...
int f(int i, int j, int k){
    ...
    int up = cost[i][j].up+f(i-1,j,k-1);
    int down = cost[i][j].down + f(i+1,j,k-1);
    int left = cost[i][j].left + f(i,j-1,k-1);
    int right = cost[i][j].right + f(i,j+1,k-1);
    return dp[i][j][k] = min({up,down,right,left});
}
...
void solve()
{
    int k;
    cin>>n>>m>>k;
    dp.resize(n,vector<vi>(m,vi(k+1,-1)));
    cost.resize(n,vector<store>(m));
    for(int i = 0;i<n; i++){
        for(int j = 0; j<m-1;j++){
            int x;
            cin>>x;
