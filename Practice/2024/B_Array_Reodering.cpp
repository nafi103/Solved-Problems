...
void solve()
{
    int n,ans = 0;
    cin>>n;
    vi two_present, not_present;
    for(int i= 0;i<n; i++){
        int x;
        cin>>x;
        if(x&1) not_present.pb(x);
        else two_present.pb(x);
    }
    vi marge = two_present;
    for(auto &x: not_present) marge.pb(x);
    for(int i = 0; i<n-1; i++){
        for(int j = i+1; j<n; j++){
            if(gcd(marge[i],2*marge[j])>1) ans++;
        }
    }
    cout<<ans<<endl;
}
