...
/****************************************************************/
vector<vi>t,parents;
vector<int>level,disSumOfChild,numofChilds,finalDist;
int mx,n;

void findFinal(int node, int parent){
    if(node!=1){
        finalDist[node] = (finalDist[parent]-numofChilds[node]) + (n-numofChilds[node]-2);
    }else{
        finalDist[node] = disSumOfChild[node];
    }
    for(auto &x: t[node]){
        if(x!=parent){
            findFinal(x,node);
...
pii findDistanceOfChild(int node, int parent){
    int childs = 0, distance = 0;
    vpi tmp;
    for(auto &x: t[node]){
        if(x!=parent){
            pii found = findDistanceOfChild(x,node);
            childs+=(found.ff+1);
            tmp.pb({x,found.ff+1});
            distance+= (found.ss+found.ff+1);
        }
    }
    disSumOfChild[node] = distance;
    numofChilds[node] = childs;
    return {childs,distance};
}
...
void solve()
{
    ...
    t.resize(n+1);
    disSumOfChild.resize(n+1,0);
    numofChilds.resize(n+1,0);
    finalDist.resize(n+1,0);
    for(int i = 1; i<n; i++){
        int u,v;
        cin>>u>>v;
        t[u].pb(v);
    ...
    findDistanceOfChild(1,-1);
    findFinal(1,-1);
    for(int i = 1; i<=n; i++) cout<<finalDist[i]<<" ";
    cout<<endl;
}
