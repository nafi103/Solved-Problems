...
int fact(int n, int ans){
    ...
    return fact(n-1,(ans*n)%mod);
}

void go(vi v, int sum){
    if(v[0]==0 and v[1]==0 and v[2]==0){
        cnt++;
        return;
    }
    if(v[1] and sum==1) go({v[0],v[1]-1,v[2]},2);
    if(v[2] and sum==2) go({v[0],v[1],v[2]-1},1);
    if(v[0]) go({v[0]-1,v[1],v[2]},sum);
}
...
void solve()
{
    ...
    if(v[1]>2) go({v[0],v[1]-2,v[2]},2);
    if(v[1] and !v[2]) go({v[0],v[1]-1,v[2]},1);
    if(v[2]) go({v[0],v[1],v[2]-1},2);
    if(!cnt) cout<<cnt<<endl;
    else{
        int ans = (((fact(v[0],1)*fact(v[1],1))%mod)*fact(v[2],1))%mod;
        ans = (ans*cnt)%mod;
        cout<<ans<<endl;
    }
}
