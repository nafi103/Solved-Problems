...
int bs(int n, int x){
    ...
    while(r-l>=0){
        int mid = (l+r)/2;
        int totEmoji = f(mid)/2;
        if(totEmoji<x) l = mid+1;
        else r = mid - 1;
    }
    return l;
}
...
int Rbs(int n, int x){
    ...
    while(r-l>=0){
        int mid = (l+r)/2;
        int totEmoji = f(n)/2 - f(mid)/2;
        if(totEmoji<x) r = mid-1;
        else l = mid+1;
    }
    return n-l+1;
}
...
void solve()
{
    int n,x;
    cin>>n>>x;
    int totEmoji = f(n-1)+n;
    if(totEmoji<=x){
        cout<<2*n - 1<<endl;
        return;
    }
    int currAns = ((x-1)/totEmoji)*n;
    x = (x-1)%totEmoji + 1;
    if((f(n)/2)<x){
        currAns+=n;
        currAns+=Rbs(n-1,x-f(n)/2);
    }else{
        currAns+=bs(n,x);
    }
    cout<<currAns<<endl;
