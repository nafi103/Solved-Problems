...
bool check(int pos, vi &v,int &n){
    vi tmp;
    for(int i = 0; i<pos; i++) tmp.pb(v[i]);
    for(int i = pos+1; i<n; i++) tmp.pb(v[i]);
    debug(tmp)
    vi gcd_seq;
    for(int i = 0; i<n-2; i++){
        gcd_seq.pb(gcd(tmp[i],tmp[i+1]));
    }
    debug(gcd_seq)
    bool flag = true;
    for(int i = 1; i<n-2; i++){
        flag&=(gcd_seq[i]>=gcd_seq[i-1]);
    }
    return flag;
}
...
void solve()
{
    ...
    readv(v);
    for(int i = 0; i<n-1; i++){
        gcd_seq.pb(gcd(v[i],v[i+1]));
    }
    debug(gcd_seq)
    vector<int>problem;
    for(int i = 1; i<n-1; i++){
        if(gcd_seq[i]<gcd_seq[i-1]){
            problem.pb(i-1);
            problem.pb(i);
            problem.pb(i+1);
            break;
