...
int f(int pos, int nonZero, bool flag, vector<int>&v){
    if(nonZero>3) return 0;
    if(pos==v.size()) return 1;
    if(dp[pos][nonZero][flag]!= -1) return dp[pos][nonZero][flag];
    dp[pos][nonZero][flag] = 0;
    for(int i = 0; i<=(flag? v[pos]:9); i++){
        dp[pos][nonZero][flag]+=f(pos+1, nonZero+(i!=0), flag&&(i==v[pos]), v);
    }
...
void solve()
{
    ...
    cin>>low>>up;
    low--;
    int usz = log10(up)+1, lsz = low==0 ? 1 : log10(low)+1;
    vector<int> u(usz), l(lsz);
    for(int i = 0; i<usz; i++){
        u[i] = up%10;
        up/=10;
    }
    ...
    int needToRemove = f(0, 0, true, l);
    memset(dp,-1,sizeof(dp));
    cout<<f(0,0,true,u) - needToRemove<<endl;
}
