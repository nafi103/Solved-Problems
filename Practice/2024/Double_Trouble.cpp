...
void solve()
{
    ...
    suff[n-1] = 1;
    for(int i=1; i<n; i++){
        if(x[i-1]+p[i-1]>=x[i]) pref[i] = 1;
        pref[i]+=pref[i-1];
    }
    for(int i = n-2; i>=0; i--){
        if(x[i+1]-p[i+1]<=x[i]) suff[i] = 1;
        suff[i]+=suff[i+1];
    }
    for(int i = 0; i<n; i++){
        int left = 0, right = 0;
        left = max(pref[i],suff[0]-suff[i] + 1);
        if(i+1<n) right = max(suff[i+1],pref[n-1]-pref[i+1]+1);
        if(left+right>=n){
            yes;
            return;
        }
    ...
    int p1 = 1;
    for(p1; x[p1-1]+p[p1-1]>=x[p1] and p1<n; p1++){
    }
    p1--;
    if(p1>=n-1){
        yes;
    ...
    if((pref[n-1]-pref[p1+1]+1==(n-(p1+1))) or (suff[p1+1]==n-1-p1)){
        yes;
        return;
    }
    p1 = n-2;
    for(p1; x[p1+1]-p[p1+1]<=x[p1] and p1>=0; p1++){
    ...
    if((pref[p1-1]==p1+1) or (suff[0]-suff[p1-1]+1==p1+1)){
        yes;
        return;
    }
    no;
