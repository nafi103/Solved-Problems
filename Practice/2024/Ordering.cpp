...
void findAll(vi &in_degree, vector<vi>&g, vi &result,vb &visited){
    bool flag = false;
    for(int i = 0; i<26; i++){
        if(in_degree[i]==0 and !visited[i]){
            for(auto &x: g[i]){
                in_degree[x]--;
            }
            result.pb(i);
            visited[i] = true;
            findAll(in_degree,g,result,visited);
            visited[i] = false;
            result.pop_back();
            for(auto &x: g[i]){
                in_degree[x]++;
    ...
    if(!flag and (int)result.size()==sz){
        ans.pb(result);
    }
}
