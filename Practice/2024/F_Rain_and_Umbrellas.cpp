...
int f(int idx, int weight){
    ...
    if(rain[idx]==true and weight==0) return INT_MAX;
    if(dp.find({idx,weight})!=dp.end()) return dp[{idx,weight}];
    dp[{idx,weight}] = INT_MAX;
    if(!rain[idx+1]) dp[{idx,weight}] = min(dp[{idx,weight}],0+f(idx+1,0));
    int currUmbrella = weight;
    if(umbrella[idx]!=0){
        if(currUmbrella) currUmbrella = min(currUmbrella,umbrella[idx]);
        else currUmbrella = umbrella[idx];
    }
    dp[{idx,weight}] = min(dp[{idx,weight}],currUmbrella+f(idx+1,currUmbrella));
...
{
    cin>>a>>n>>m;
    rain.assign(a+2,false);
    umbrella.assign(a+1,0ll);
    for(int i = 0; i<n; i++){
        int l,r;
        cin>>l>>r;
        for(int i = l; i<=r; i++) rain[i] = true;
    }
    for(int i = 0; i<m; i++){
        int idx,w;
        cin>>idx>>w;
