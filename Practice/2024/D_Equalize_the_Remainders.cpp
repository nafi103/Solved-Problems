...
void solve()
{
    ...
    for(auto &[f,s]: mp){
        if(s>final) {
            q.push_back({f,s-final});
            s = final;
        }
    }
    for(int i = 0; i<m; i++){
        int currHave = mp[i];
        if(currHave==final) continue;
        int need = final-currHave;
        while(need--){
            bool flag = true;
            int currEle = q.front().ff;
            if(currEle<i){
                ans+=(i-currEle);
                add[currEle].pb(i-currEle);
            }else{
                currEle = q.back().ff;
                flag = false;
                ans+=(m-currEle+i);
                add[currEle].pb(m-currEle+i);
            }
            if(flag){
                q.front().ss--;
                if(q.front().ss==0) q.pop_front();
            }else{
                q.back().ss--;
                if(q.back().ss==0) q.pop_back();
            }
        }
    }
    for(auto &x: v){
