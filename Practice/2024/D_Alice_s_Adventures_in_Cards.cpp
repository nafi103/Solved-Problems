...
void solve()
{
    int n;
    cin>>n;
    vi q(n+1), k(n+1), j(n+1);
    input(q,n); input(k,n); input(j,n);
    vector<int>possible(n+1,false);
    vector<pair<char,int>>how_reached(n+1);
    possible[1] = true;
    pii queen = {q[1],1}, king = {k[1],1}, jack = {j[1], 1};
    for(int i = 2; i<=n; i++){
        if(queen.ff>q[i] or king.ff>k[i] or jack.ff>j[i]){
            possible[i] = true;
            if(q[i]>queen.ff){
                queen = {q[i], i};
            }else{
                how_reached[i] = {'q',queen.ss};
            }
            if(k[i]>king.ff){
                king = {k[i], i};
            }else{
                how_reached[i] = {'k', king.ss};
            }
            if(j[i]>jack.ff){
                jack = {j[i], i};
            }else{
                how_reached[i] = {'j',jack.ss};
            }
        }
    }
    if(possible[n]) {
        ...
        int pos = n;
        while(pos>1){
            ans.pb({how_reached[pos].ff, pos});
            pos = how_reached[pos].ss;
        }
        reverse(all(ans));
