...
void solve()
{
    int n;
    cin>>n;
    if(binary_search(all(dp1),n) or binary_search(all(dp2),n)) yes;
    else no;
}
...
int32_t main()
{
    ...
    dp1.pb(2);
    dp2.pb(4);
    while(dp1.back()<INT_MAX){
        int next = 2*dp1.size()+1;
        dp1.pb(dp1.back()+next*2);
    }
    debug(dp1.size());
    while(dp2.back()<INT_MAX){
        int next = 2*dp2.size()+1;
        dp2.pb(dp2.back()+next*4);
    }
    debug(dp2.size());
    int t = 1;
    cin >> t;
