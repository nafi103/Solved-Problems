...
void solve()
{
    int n,m;
    cin>>n>>m;
    vector<vi>b(n,vi(m)),a(n,vi(m));
    vi row(n*m+1),column(n*m+1),countA(n*m+1,1),countB(n*m+1,1);
    for(int i = 0; i<n; i++){
        for(int j = 0; j<m; j++){
            cin>>a[i][j];
            row[a[i][j]] = i;
    ...
    for(auto &x:b) readv(x);
    debug(column)
    for(int j = 0; j<m; j++){
        int colA = column[b[0][j]];
        for(int i = 0; i<n; i++){
            countB[b[i][j]]--;
        }
        for(int i = 0; i<n; i++){
            countA[a[i][colA]]--;
            if(countA[a[i][colA]]!=countB[a[i][colA]]){
                no;
                return;
            }
        }
    ...
    for(int i = 0; i<n; i++){
        ...
        for(int j =0; j<m; j++){
            countA[a[rowA][j]]--;
            if(countA[a[rowA][j]]!=countB[a[rowA][j]]){
                no;
                return;
            }
        }
