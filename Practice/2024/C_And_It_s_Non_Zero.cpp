...
void solve()
{
    ...
    int x = 2;
    while(x<=INT_MAX){
        int rightBits = ((r+1)/x)*(x/2) + (max(0ll,(r+1)%x - x/2));
        int leftBits = ((l)/x)*(x/2) + (max(0ll,(l)%x - x/2));
        debug(x)
        debug(leftBits) debug(rightBits)
        if(!rightBits and !leftBits){
            break;
        }
        ans = min(ans, (r-l+1) - (rightBits-leftBits));
        x<<=1;
    }
    cout<<ans<<endl;
}
