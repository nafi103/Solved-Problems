...
vector<vi>t,parents;
vector<int>level;
int mx,firstDiameter = 1, lOfDnode = 0;

void dfs1(int node, int parent, int cLevel, int &secondDiameter){
    if(cLevel>lOfDnode){
        secondDiameter = node;
        lOfDnode = cLevel;
    }
    for(auto &x: t[node]){
        if(x!=parent){
            dfs1(x,node,cLevel+1,secondDiameter);
        }
    }
}
...
void solve()
{
    ...
    lOfDnode = 0;
    int secondDiameter;
    dfs1(firstDiameter,-1, 0,secondDiameter);
    debug(secondDiameter)
    for(int i = 1; i<=n; i++){
        cout<<max(distance(i,firstDiameter),distance(i,secondDiameter))<<" ";
    }
    cout<<endl;
}
