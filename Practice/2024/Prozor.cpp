...
void solve()
{
    ...
    int x1,y1,x2,y2,kill = 0;
    for(int i = 1; i<=n-k+1; i++){
        int nexti = i+k-1;
        for(int j = 1; j<=m-k+1; j++){
            int nextj = j+k-1;
            int flys = grid[nexti-1][nextj-1] - grid[nexti-1][j] - grid[i][nextj-1] + grid[i][j];
            if(flys>kill){
                kill = flys;
                x1 = i,y1 = j,x2 = nexti, y2 = nextj;
            }
        }
    }
    picture[x1][y1] = picture[x2][y2] = picture[x2][y1] = picture[x1][y2] = '+';
    for(int i = x1+1; i<x2; i++){
        picture[i][y1] = picture[i][y2] = '|';
    }
    for(int i = y1+1; i<y2; i++){
        picture[x1][i] = picture[x2][i] = '-';
    }
    cout<<kill<<endl;
    for(int i = 1; i<=n; i++){
        for(int j = 1; j<=m; j++) cout<<picture[i][j];
        cout<<endl;
    }
}
