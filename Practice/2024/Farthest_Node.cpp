...
/****************************************************************/
vector<vi>t;
vector<int>found_node;

pair<int,int> findFarthestNode(int node, int parent){
    pair<int,int> farthest_node = {0,node};
    for(auto child: t[node]){
        if(child==parent) continue;
        farthest_node = max(farthest_node,findFarthestNode(child,node));
    }
    found_node[node] = farthest_node.ss;
    return {farthest_node.ff+1, farthest_node.ss};
}
...
void solve()
{
    ...
    cin>>n;
    t.resize(n+1);
    found_node.resize(n+1,0);
    for(int i = 1; i<n; i++){
        cin>>u>>v;
        t[u].pb(v);
        t[v].pb(u);
    }
    findFarthestNode(1,1);
    writev(found_node);
}
