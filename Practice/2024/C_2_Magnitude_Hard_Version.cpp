...
void solve()
{
    ...
    vi v(n);
    readv(v);
    vector<vi>dp(n,vi(2,0)),cnt(n,vi(2,0));
    cnt[0][0] = v[0]>=0?2:1;
    cnt[0][1] = v[0]>=0?2:1;
    dp[0][0] = abs(v[0]);
    dp[0][1] = v[0];
    for(int i= 1; i<n; i++){
        dp[i][0] = dp[i-1][0]+ v[i];
        dp[i][1] = dp[i-1][1]+ v[i];
        dp[i][0] = max(dp[i][0],abs(dp[i][1]));
        debug(dp[i][0])
        debug(dp[i][1])
        if (dp[i][1] == dp[i-1][0] + v[i])
            cnt[i][1] = (cnt[i][1] + cnt[i-1][0]) % mod;
        if (dp[i][1] == abs(dp[i-1][0] + v[i]))
            cnt[i][1] = (cnt[i][1] + cnt[i-1][0]) % mod;
        if (dp[i][0] == dp[i-1][0] + v[i])
            cnt[i][0] = (cnt[i][0] + cnt[i-1][0]) % mod;
        if (dp[i][0] == abs(dp[i-1][0] + v[i]))
            cnt[i][0] = (cnt[i][0] + cnt[i-1][0]) % mod;
