...
bool rival(int &b1, int &b2){
    return (b2-b1==1) or (b2==0 and b1==n-1);
}

bool lost(int i, int j){
    for(int k = 0; k<4; k++){
        int ni = i+x[k], nj = j+y[k];
        if(valid(ni,nj) and rival(grid[ni][nj], grid[i][j])){
            return true;
        }
    }
    return false;
...
{
    grid.resize(r,vi(c));
    for(auto &v: grid) readv(v);
    vector<vector<bool>>taken(r,vector<bool>(c,false));
    while(k--){
        for(int i = 0; i<r; i++){
            for(int j = 0; j<c; j++){
                if(lost(i,j)) taken[i][j] = true;
            }
        }
        for(int i = 0; i<r; i++){
            for(int j = 0; j<c; j++){
        ...
        if(k) taken.assign(r,vector<bool>(c,false));
    }
    for(int i = 0; i<r; i++){
        for(int j = 0; j<c; j++){
            if(j) cout<<" ";
