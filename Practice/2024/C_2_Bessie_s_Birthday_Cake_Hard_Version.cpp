...
void solve()
{
    ...
    if((v[0]==1 and v[x-1]==n-1) or (v[0]==2 and v[x-1]==n)) ans++;
    vector<pair<int,int>>egons,ogons;
    for(int i = 0; i<x-1; i++){
        int nodes = v[i+1]-v[i]+1;
        if(nodes>3){
            int needToMx = (nodes-2)/2;
            if(nodes&1) ogons.pb({needToMx,nodes-2});
            else egons.pb({needToMx,nodes-2});
        }
    }
    int last = v[0]-v[x-1]+n+1,lmx;
    if(last>3){
        lmx = (last-2)/2;
        ((last%2==0)? egons: ogons).push_back({lmx,last-2});
    }
    sort(egons.begin(),egons.end());
    sort(ogons.begin(),ogons.end());
    debug(egons) debug(ogons);
    for(auto [f,s]: ogons){
        if(y>=f){
            ans+=s;
            y-=f;
