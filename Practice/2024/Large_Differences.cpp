...
void solve()
{
    ...
    int currAdd = 0, pos = -1,change = 0,add = 0;
    for(int i = 0; i<n; i++){
        int now = ((i>0)? abs(v[i]-v[i-1]): 0)+(i<n-1? abs(v[i]-v[i+1]):0);
        debug(now)
        int tot = max(((i>0)? abs(1-v[i-1]): 0)+(i<n-1? abs(1-v[i+1]):0),((i>0)? abs(k-v[i-1]): 0)+(i<n-1? abs(k-v[i+1]):0));
        debug(tot)
        if(tot>=now and tot-now>currAdd){
            currAdd = tot-now;
            add = tot;
            change = ((i>0)? abs(v[i]-v[i-1]): 0)+(i<n-1? abs(v[i]-v[i+1]):0);
            pos = i;
        }
    }
    debug(pos)
