...
void solve()
{
    ...
    cin>>x>>y>>z>>n;
    int totalClass = x*y;
    int classNeeded = (totalClass*60+99)/100;
    int classLeft = (x-z)*y;
    if(classNeeded>classLeft+n){
        cout<<"No"<<endl;
        return;
    }
    cout<<"Yes"<<endl;
    vi ans;
    int attendClass = max(0ll,classNeeded - n);
    for(int i = 0; i<x-z; i++){
        ans.pb(min(attendClass,y));
        attendClass-=min(attendClass,y);
    }
    reverse(all(ans));
    writev(ans);
}
