...
int fellLeft(vpi &v,vector<vi>&dp, int i, int lastLeft, int lastRight){
    int leftVal = dp[i-1][0] + (v[i].ff-v[i].ss>lastLeft?1:0);
    int rightVal = dp[i-1][1] + (v[i].ff-v[i].ss>lastRight?1:0);
    dp[i][0] = max(leftVal,rightVal);
    return v[i].ff;
}

int fellRight(vpi &v,vector<vi>&dp, int i, int lastLeft, int lastRight){
    if(i==n-1 or v[i].ff+v[i].ss<v[i+1].ff){
        dp[i][1] = max(dp[i-1][0]+1,dp[i-1][1]+1);
        return v[i].ff+v[i].ss;
    }
    return v[i].ff;
}
...
void solve()
{
    ...
    int lastLeft = v[0].ff,lastRight = v[0].ff;
    vector<vi>dp(n,vi(2));
    dp[0][0] = 1;
    dp[0][1] = 1;
    for(int i = 1; i<n; i++){
        lastLeft = fellLeft(v,dp,i,lastLeft,lastRight);
        lastRight = fellRight(v,dp,i,lastLeft,lastRight);
    }
