...
void solve()
{
    ...
    readv(v);
    sort(all(v));
    int totalS = accumulate(all(v),0LL);
    int m;cin>>m;
    while(m--){
        int attack,defense;
        cin>>defense>>attack;
        int pos = lower_bound(all(v),defense) - v.begin();
        if(pos==n or pos==0){
            if(pos==n) pos--;
            int remStr = totalS - v[pos],chosenSol = v[pos];
            cout<<max(0ll,defense - chosenSol) + max(0ll,attack - remStr)<<endl;
        }else{
            int solder1 = v[pos], solder2 = v[pos-1];
            int frS1 = totalS - solder1, frS2 = totalS - solder2;
            int ans = min(max(0ll, defense - solder1) + max(0ll, attack - frS1), max(0ll, defense - solder2) + max(0ll, attack - frS2));
            cout<<ans<<endl;
        }
    }
}
