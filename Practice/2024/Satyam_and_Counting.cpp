...
void solve()
{
    ...
    int ans = 0;
    for (auto [point, c] : Zero) {
        if (One.count(point)) {
            ans += One.size() + Zero.size() - 2;
        }

        if (One.count(point + 1) and Zero.count(point + 2)) {
            ans += 1;
        }
    }
    for (auto [point, c] : One) {
        if (Zero.count(point - 1)  and One.count(point - 2)) {
            ans += 1;
        }
    }
    cout<<ans<<endl;
