...
void dfs(int node, vector<vi>&g,vector<bool>&visited, map<int,string>&name){
    visited[node] = true;
    for(auto &x: g[node]){
        if(!visited[x]) dfs(x,g,visited,name);
    }
    ans.pb(name[node]);
}
...
void solve()
{
    ...
    for(int i = 0; i<n; i++){
        ...
        while(s>>str){
            ...
            if(!idx.count(str)) {name[(int)idx.size()] = str; idx[str] = (int)idx.size();}
            if(!parentFound){
                parentFound = true;
                parent = idx[str];
            }else{
