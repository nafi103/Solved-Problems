...
vector<int> a(MAXN),perm(MAXN);

int findMax(int pos, int k){
    vector<bool>visited(n+1,false);
    int mx = 0, val = 0;
    while(k and !visited[pos]){
        mx = max(mx, val+a[pos]*k);
        visited[pos] = true;
        k--;
        val+=a[pos];
        pos = perm[pos];
...
void solve()
{
    ...
    vector<bool>visited(n+1,false);
    int mxa = findMax(pa,k),mxb = findMax(pb,k);
    if(mxa>mxb) cout<<"Bodya"<<endl;
    else if(mxa==mxb) cout<<"Draw"<<endl;
