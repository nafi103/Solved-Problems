...
/****************************************************************/
vector<vector<pii>>t;
vector<vi>parents,dist;
vector<int>level;
int mx;

void dfs(int node, int parent, int cLevel, int wt){
    if(node!=1) dist[0][node] = wt;
    level[node] = cLevel;
    parents[0][node] = parent;
    for(auto &[x,w]: t[node]){
        if(x!=parent) dfs(x,node, cLevel+1,w);
    }
}
...
int kthParentAtoB(int k, int a, int b){
    int LCA = lca(a,b);
    int distanceAtoLCA = level[a] - level[LCA];
    int distanceBtoLCA = level[b] - level[LCA];
    if(k<=distanceAtoLCA) return kthParent(a,k);
    else{
        int kForB = distanceBtoLCA - (k-distanceAtoLCA);
        return kthParent(b,kForB);
    }
}
...
int distanceAtoB(int a, int b){
    int LCA = lca(a,b);
    debug(LCA)
    return distance(a,level[a]-level[LCA])+distance(b,level[b]-level[LCA]);
}
...
void solve()
{
    ...
    cin>>n;
    mx = log2(n)+1;
    parents.resize(mx,vi(n+1,-1));
    dist.resize(mx,vi(n+1,-1));
    t.resize(n+1);
    level.resize(n+1);
    for(int i = 1; i<n; i++){
        int u,v,w;
    ...
    for(int i = 1; i<mx; i++){
        for(int j = 1; j<=n; j++){
            ...
            if(prevParent!=-1){
                ...
                if(dist[i-1][prevParent]!=-1) dist[i][j] = dist[i-1][j] + dist[i-1][prevParent];
            }
        }
    }
    debug(dist)
    ...
    while(str!="DONE"){
        if(str=="KTH"){
            int k,a,b;
            cin>>a>>b>>k;
            cout<<kthParentAtoB(k-1,a,b)<<endl;
        }else{
            int a,b;
            cin>>a>>b;
            cout<<distanceAtoB(a,b)<<endl;
        }
        cin>>str;
    }
    t.clear();
