...
int dfs(int i, int j, vector<vector<bool>>&visited,vector<vector<char>>&grid){
    visited[i][j] = true;
    int ans = (grid[i][j]=='G');
    for(int k = 0; k<4; k++){
        int ni = i+y[k], nj = j+x[k];
        if(valid(ni,nj) and grid[ni][nj]=='T') return ans;
    }
    for(int k = 0; k<4; k++){
        int ni = i+y[k], nj = j+x[k];
        if(valid(ni,nj) and grid[ni][nj]!='#' and !visited[ni][nj]){
            ans+=dfs(ni,nj,visited,grid);
        }
    }
    return ans;
}
...
int32_t main()
{
    ...
    cout<<dfs(si,sj,visited,grid)<<endl;
}