...
int f(int i, int lastColor){
    if(i==n) return 0;
    if(lastColor!=-1 and dp[i][lastColor]!=-1) return dp[i][lastColor];
    int frCpu1, frCpu2;
    frCpu1 = f(i+1,v[i-1])+(v[i]==lastColor? hot[v[i]]: cold[v[i]]);
    frCpu2 = f(i+1,lastColor)+(v[i]==v[i-1]? hot[v[i]]: cold[v[i]]);
    return dp[i][lastColor] = min(frCpu1,frCpu2);
}
