...
void solve()
{
    ...
    while(q--){
        int l, r, k;
        cin>>l>>r>>k;
        int o_c = ones[r] - ones[l-1],t_c = twos[r] - twos[l-1];
        int range = r-l+1;
        if(o_c>t_c) swap(o_c,t_c);
        int otherValues = range - (o_c+t_c);
        if(otherValues>=k){
            int need = t_c-o_c;
            int canIncrease = min(k,need);
            o_c+=canIncrease;
            k-=canIncrease;
            int bothIncrease = k/2;
            o_c+=bothIncrease;
            t_c+=bothIncrease;
            if(k&1) o_c++;
            cout<<o_c*t_c<<endl;
            ...
            k-=otherValues;
            if(o_c+k>t_c-k){
                cout<<(range/2)*((range+1)/2)<<endl;
            }else{
                cout<<(o_c+k)*(t_c-k)<<endl;
            }
        }
    }
}
