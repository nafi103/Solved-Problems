...
int f(int n){
    // state: dp[i] = the minimum coins needed to reach i
    // base case: if we reach any given coin the simply return 1 and if less than n then INT_MAX
    if(n<mn) return INT_MAX;
    if(dp[n]!=INT_MAX) return dp[n];
    // transition : dp[i] = min(dp[i],{dp[i-v[1]]+1,dp[i-v[2]]+1,....,dp[i-v[n-1]]+1})
    for(auto &x: v){
        dp[n] = min(dp[n], f(n-x) + 1);
    }
    return dp[n];
