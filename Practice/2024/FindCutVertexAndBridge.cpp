...
vector<vector<bool>>cut_bridge;
int dfsRoot,dfsCnt,rootChild;
...
void resizeAll(int n){
    ...
    dfs_low.resize(n+1);
    cut_vertex.resize(n+1,false);
    parent.resize(n+1,-1);
    cut_bridge.resize(n+1,vector<bool>(n+1,false));
}
...
void dfs(int node){
    ...
    for(auto &x: g[node]){
        if(dfs_num[x]==unvisited){
            parent[x] = node;
            if(node==dfsRoot) rootChild++;
            dfs(x);
            if(dfs_low[x]>=dfs_num[node]) cut_vertex[node] = true;
            if(dfs_low[x]>dfs_num[node]) {
                cut_bridge[node][x] = true;
                cut_bridge[x][node] = true;
            }
...
void solve()
{
    ...
    for(int i = 1; i<=n; i++){
        if(dfs_num[i]==unvisited){
            ...
            rootChild = 0;
            dfs(i);
            cut_vertex[dfsRoot] = (rootChild>1);
        }
    }
}
