...
int f(int pos, int x,vector<int>&nums2,vector<int>&nums1){
    if(x==mx) return 0;
    if(dp[pos][x]!=-1) return dp[pos][x];
    dp[pos][x] = INT_MAX;
    for(int i = 0; i<n; i++){
        if(!(x&(1<<i))) dp[pos][x] = min(dp[pos][x], (nums2[i]^nums1[pos])+f(pos+1,(x|(1<<i)),nums2,nums1));
    }
    return dp[pos][x];
}
...
void solve()
{
    ...
    cout<<minimumXORSum(nums1,nums2)<<endl;
    debug(dp);
}
