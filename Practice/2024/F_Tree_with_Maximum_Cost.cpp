...
/****************************************************************/
vector<vi>t;
vector<int>valSumOfChild,sumOfChilds,finalVal,weight;
int mx,n,totSumofChild = 0;

void findFinal(int node, int parent){
    if(node!=1){
        finalVal[node] = finalVal[parent] - sumOfChilds[node] + totSumofChild - sumOfChilds[node];
    }else{
        finalVal[node] = valSumOfChild[node];
    }
    for(auto &x: t[node]){
        if(x!=parent){
            findFinal(x,node);
...
void findValueFromChild(int node, int parent){
    for(auto &x: t[node]){
        if(x!=parent){
            findValueFromChild(x, node);
            sumOfChilds[node] += sumOfChilds[x];
            valSumOfChild[node] += valSumOfChild[x] + sumOfChilds[x];
        }
    }
    sumOfChilds[node] += weight[node];
}
...
void solve()
{
    ...
    t.resize(n+1);
    weight.resize(n+1);
    valSumOfChild.resize(n+1,0);
    sumOfChilds.resize(n+1,0);
    finalVal.resize(n+1,0);
    for(int i = 1; i<=n; i++) {cin>>weight[i];totSumofChild+=weight[i];}
    for(int i = 1; i<n; i++){
        int u,v;
        cin>>u>>v;
        t[u].pb(v);
    ...
    findValueFromChild(1,-1);
    findFinal(1,-1);
    int ans = 0;
    for(int i = 1; i<=n; i++) ans = max(finalVal[i],ans);
    cout<<ans<<endl;
}
