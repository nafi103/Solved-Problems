...
/****************************************************************/
int allPossible,chosenSize;

vpi ansRanges;

string makeBirary(int n){
    string str = "";
    while(str.size()<chosenSize){
        str.pb('0'+(n&1));
        n>>=1;
    }
    reverse(all(str));
...
void solve()
{
    ...
    allPossible = 1<<(chosen.size());
    long long maxAns = n*n;
    string maxStr = string(chosen.size(),'0');
    for(int i = 1; i<allPossible; i++){
        string str = makeBirary(i);
        vpi nw;
        for(int i = 0; i<chosenSize; i++){
            if(str[i]=='1') nw.pb(chosen[i]);
        }
        debug(nw);
        long long cnt = 0;
        cnt+=(nw[0].ss)*(nw[0].ss);
        ...
        for(int i = 1; i<nw.size(); i++){
            cnt+=(nw[i].ss-nw[i-1].ss-1)*(nw[i].ss-nw[i-1].ss-1);
            cnt+=nw[i].ff;
        }
        cnt+=(n-1-(nw.back().ss))*(n-1-(nw.back().ss));
        if(cnt>=maxAns){
            if(cnt==maxAns){
                int ocnt = 0,ncnt = 0;
                for(auto x: maxStr) if(x=='1') ocnt++;
                for(auto x: str) if(x=='1') ncnt++;
                if(ncnt>ocnt) maxStr = str;
            }else{
                maxAns = cnt;
                maxStr = str;
            }
    ...
    vi finaChosen;
    for(int i = 0; i<chosenSize; i++){
        if(maxStr[i]=='1') finaChosen.pb(chosen[i].ss);
    }
    debug(finaChosen);
    int last = 0;
    vpi ranges;
    for(auto x: finaChosen){
        if(last==x){
            last++;
        }else{
            ranges.pb({last+1,x});
    ...
    if(last<=n-1) ranges.pb({last+1,n});
    debug(ranges);
    for(auto &[f,s]: ranges){
        ansRanges.pb({f,s});
        makeMax(f,s);
