...
void solve()
{
    ...
    while(k>0){
        ...
        for(int i = 0; i<n; i++){
            if(chosenChar<str[i]) {
                if(str[i]-'a'<=k) chosenChar = str[i];
                else{
                    if(chosenChar=='a') chosenChar = str[i];
                    break;
                }
            }
        }
        if(chosenChar == 'a') break;
        int diff = min((int)chosenChar-'a',k);
        k-=diff;
        char changeChar = chosenChar - diff;
        for(int i= 0; i<n; i++){
            if(str[i]<=chosenChar and str[i]>changeChar) str[i] = changeChar;
        }
        chosenChar = 'a';
    }
    cout<<str<<endl;
