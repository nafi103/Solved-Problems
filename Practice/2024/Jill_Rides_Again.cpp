...
void solve(int z)
{
    ...
    vi v(n-1);
    readv(v);
    int currSum = 0,i = 0, j = 0, ansi = 0, ansj = 0,sum = 0;
    while(j<n-1){
        currSum+=v[j];
        if(currSum>=sum){
            if(currSum>sum or (currSum==sum and j-i>ansj-ansi)){
                ansi = i;
                ansj = j;
            }
            sum = currSum;
    ...
    if(sum) cout<<"The nicest part of route "<<z<<" is between stops "<<ansi+1<<" and "<<ansj+2<<endl;
    else cout<<"Route "<<z<<" has no nice parts"<<endl;
}
