...
int calc(int idx, vi &v){
    int sum = (idx/n)*tot,start = (idx/n)+1;
    if(idx%n==0) return sum;
    int remEle = idx%n, remFromStart = n-start+1,firstToRem;
    int firstEnd = min(n,start+remEle-1);
    if(remEle>remFromStart) firstToRem = remEle-remFromStart;
    else firstToRem = 0;
    return sum+(v[firstEnd]-v[start-1])+v[firstToRem];
}
