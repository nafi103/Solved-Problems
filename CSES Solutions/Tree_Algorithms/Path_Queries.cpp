...
/****************************************************************/
vi value,inTime,outTime,forPath,sTree;
vector<vi> t;

void eular_tour(int node, int parent, int &time){
    forPath.pb(value[node]);
    inTime[node] = time++;
    for(auto &x: t[node]){
        if(x!=parent){
            eular_tour(x,node,time);
        }
    }
    forPath.pb(-value[node]);
    outTime[node] = time++;
...
void solve()
{
    ...
    while(q--){
        ...
        if(type==1){
            int node,val;
            cin>>node>>val;
            update(1,1,sz-1,inTime[node],val);
            update(1,1,sz-1,outTime[node],-val);
        }else{
            int node;
            cin>>node;
            cout<<query(1,1,sz-1,1,outTime[node]-1)<<endl;
        }
    }
}
