...
/****************************************************************/
vector<vi>t,parents;
vector<int>level,passThrough;
int mx;

int addDfs(int node, int parent){
    int val = 0;
    for(auto &x: t[node]){
        if(x!=parent){
            val+=(addDfs(x,node));
...
void addVal(int a, int b){
    ...
    passThrough[b]++;
    passThrough[LCA]--;
    if(parents[0][LCA]!=-1) passThrough[parents[0][LCA]]--;
    debug(passThrough)
}
...
void solve()
{
    ...
    for(int i = 1; i<mx; i++){
        ...
        for(int j = 1; j<=n; j++){
            ...
            if(prevParent!=-1) {flag = false; parents[i][j] = parents[i-1][prevParent];}
        }
        if(flag) break;
    }
    while(q--){
    ...
    addDfs(1,-1);
    for(int i = 1; i<=n; i++) cout<<passThrough[i]<<" ";
    cout<<endl;
}
