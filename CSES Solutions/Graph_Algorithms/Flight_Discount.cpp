...
/****************************************************************/
void dijsktra(int src,vi &distance,vpi v[])
{
    set<pii>pq;
    distance[src] = 0;
    pq.insert({0, src});
    while (!pq.empty())
    {
        ...
        {
            if (distance[nbr] > p.ff + wt)
            {
                pq.erase({distance[nbr],nbr});
                distance[nbr] = p.ff + wt;
                pq.insert({distance[nbr], nbr});
            }
        }
    }
}
...
void solve()
{
    ...
    cin>>n>>m;
    vpi nrv[n],rv[n];
    vector<int>distanceA(n,inf), distanceB(n,inf);
    ...
    dijsktra(0,distanceA,nrv);
    dijsktra(n-1,distanceB,rv);
    for(auto &x: edges){
        int a = x[0], b = x[1], dis = x[2];
        ans = min(ans, distanceA[a]+distanceB[b]+dis/2);
    }
    cout<<ans<<endl;
}
