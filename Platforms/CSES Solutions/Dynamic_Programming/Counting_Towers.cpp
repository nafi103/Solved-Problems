...
/****************************************************************/
vector<vector<int>>dp(MAXN,vi(2,-1));

int f(int n, int state){
    if(dp[n][state]!=-1) return dp[n][state];
    if(state==0) dp[n][state] = (2*f(n-1,0)+f(n-1,1))%mod;
    else dp[n][state] = (4*f(n-1,1)+f(n-1,0))%mod;
    return dp[n][state];
}
...
void solve()
{
    int n;
    cin>>n;
    if(dp[n][0]!=-1) {cout<< (dp[n][0]+dp[n][1])%mod<<endl;return;}
    dp[1][0] = 1;
    dp[1][1] = 1;
