...
int findAns(string &a, string &b, int pa, int pb){
    if(dp[pa+1][pb+1]!=INT_MAX) return dp[pa+1][pb+1];
    if(a[pa]==b[pb]) return dp[pa+1][pb+1] = findAns(a,b,pa-1,pb-1);
    return dp[pa+1][pb+1] = min({1+findAns(a,b,pa,pb-1),1+findAns(a,b,pa-1,pb),1+findAns(a,b,pa-1,pb-1)});
}
