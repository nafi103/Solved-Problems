...
void dfs(vector<vector<char>>&arr,vector<vector<bool>>&visited,int i, int j){
    if(i<0 or i>=n or j<0 or j>=m or visited[i][j] or arr[i][j]=='#') return;
    visited[i][j] = true;
    rep(k,0,4){
...
int countRoom(vector<vector<char>>&arr,vector<vector<bool>>&visited){
    int cnt = 0;
    rep(i,0,n){
        rep(j,0,m){
...
void solve()
{
    ...
    cout<<countRoom(arr,visited)<<endl;
}
