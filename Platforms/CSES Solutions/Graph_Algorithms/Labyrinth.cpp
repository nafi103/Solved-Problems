...
string dfs(int i, int j, int parI, int parJ, string curr){
    if(i==n or j==m or i<0 or j<0) return {};
    if(visited[i][j] or v[i][j]=='#') return {};
    if(v[i][j]=='B') return curr;
    visited[i][j] = true;
    for(int k = 0; k<4; k++){
        if(i+diry[k]!=parI or j+dirx[k]!=parJ){
            string found = dfs(i+diry[k],j+dirx[k],i,j,curr+add[k]);
