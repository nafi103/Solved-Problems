...
/****************************************************************/
vector<int>a(MAXN),b(MAXN),pref = {0},parent(MAXN),ans(MAXN);
vector<vi>t;

void dfs(int node, int aiSum){
    debug(node);
    aiSum+=a[node];
    int biSum = pref.back() + b[node];
    if(node!=1) pref.pb(biSum);
    debug(aiSum)
    int len = upper_bound(all(pref),aiSum) - pref.begin() - 1;
    ans[node] = len;
    debug(len) debug(pref)
    for(auto &x: t[node]){
        if(x!=parent[node]){
            dfs(x,aiSum);
            while(pref.back()!=biSum) pref.pop_back();
        }
    }
}
