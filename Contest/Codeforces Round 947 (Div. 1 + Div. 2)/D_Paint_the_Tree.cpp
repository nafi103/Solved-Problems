...
/****************************************************************/
vector<vi>t,parents;
vector<int>level,maxHeightBelow;
int mx,currAns;

int findFarthestNode(int node, int parent){
    int farthest_node = 0;
    for(auto child: t[node]){
        if(child==parent) continue;
        farthest_node = max(farthest_node,findFarthestNode(child,node));
    }
    maxHeightBelow[node] = farthest_node;
    return farthest_node+1;
}
...
void findAns(int node, int parent,int &ttime){
    currAns = max(currAns,ttime++);
    int mxHeight = -1,mxNode = -1;
    for(auto &x: t[node]){
        if(x!=parent){
            if(maxHeightBelow[x]>mxHeight){
                mxHeight = maxHeightBelow[x];
                mxNode = x;
            }
        }
    }
    for(auto &x: t[node]){
        if(x!=parent and x!= mxNode) findAns(x,node,ttime);
    }
    if(mxNode!=-1) findAns(mxNode,node,ttime);
    ttime++;
}
...
void solve()
{
    ...
    for(int i = 1; i<mx; i++){
        for(int j = 1; j<=n; j++){
            int prevPar = parents[i-1][j];
            if(prevPar!=-1) parents[i][j] = parents[i-1][prevPar];
        }
    }
    int disFromAtoB = level[b];
    int add = (disFromAtoB+1)/2;
    int bWillStart = kthParent(b,add);
    findFarthestNode(bWillStart,-1);
    int ttime = 0;
    findAns(bWillStart,-1,ttime);
    cout<<currAns+add<<endl;
    t.clear();
    maxHeightBelow.clear();
    level.clear();
    parents.clear();
}
