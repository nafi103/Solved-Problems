...
vi k;
 int check(int day){
    map<int,int>mx;
    for(auto &[d,t]:discount){
        if(d<=day) mx[t] = max(mx[t],d);
    }
    vector<vi> lastOff(day+1);
    for(auto &[t,d]:mx) lastOff[d].pb(t);
    vi tk = k;
    int burles = 0, i = 1, remain = accumulate(all(k),0ll);
    while(i<=day){
        burles++;
        for(auto &x: lastOff[i]){
            if(burles>=tk[x]){
                remain-=tk[x];
                burles-=tk[x];
                tk[x] = 0;
 ...
 void solve(){
    ...
    cout<<bs(1,2*accumulate(all(k),0ll))<<endl;
}
 int32_t main()
{
    fastIO;
