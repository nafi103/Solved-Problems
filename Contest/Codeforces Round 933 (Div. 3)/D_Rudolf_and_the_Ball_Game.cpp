...
int getRandomNumber(int l, int r) {return uniform_int_distribution<int>(l, r)(rng);} 
 /****************************************************************/
int n, m, x;
 void addClockWise(vector<int> &v, int dist)
{
    for(auto &x: v){
        x = (x+dist)%n;
        if(x==0) x = n;
 ...
 void addAntiClockWise(vector<int> &v, int dist)
{
    for(auto &x: v){
        x = (x-dist+n)%n;
        if(x==0) x = n;
 ...
 void addBoth(vector<int>&v,int dis){
    vector<int>tmp = v;
    addClockWise(v,dis);
    addAntiClockWise(tmp,dis);
 ...
 void solve()
{
    ...
    while(m--){
        ...
        char c;
        cin>>dis>>c;
        if(c=='0') addClockWise(v,dis);
        else if(c=='1') addAntiClockWise(v,dis);
        else addBoth(v,dis);
    }
    make_unique(v);
    cout<< v.size() << endl;
