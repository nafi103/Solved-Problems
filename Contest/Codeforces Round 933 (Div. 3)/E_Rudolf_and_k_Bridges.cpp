...
void f(int i){
    deque<int>trackMin;
    dp[i][0] = 1;
    trackMin.push_back(0); 
    for(int j = 1; j<m; j++){
        while(trackMin.front()<=j-d-2) trackMin.pop_front();
        dp[i][j] = dp[i][trackMin.front()] + v[i][j] + 1;
        while(!trackMin.empty() and dp[i][j]<dp[i][trackMin.back()]) trackMin.pop_back();
