...
void findAns(int node){
    dp[node][0] = INT_MAX;
    if(str[node]!='S') dp[node][1] = 0;
    else dp[node][1] = INT_MAX;
    if(str[node]!='P') dp[node][2] = 0;
    else dp[node][2] = INT_MAX;
    int tot = 0;
    for(auto x: t[node]){
        findAns(x);
        dp[node][1] = dp[node][1]+min({dp[x][0],dp[x][1],dp[x][2]+1});
        dp[node][2] = dp[node][2]+min({dp[x][0],dp[x][1]+1,dp[x][2]});
        tot+=dp[node][0];
    }
    if(str[node]!='C') tot = INT_MAX;
    dp[node][0] = min({tot,dp[node][1]+1,dp[node][2]+1});
    if(node==1){
