...
ld f(vi v){
    ...
    if(dp[v[1]][v[2]][v[3]]!=-1.0) return dp[v[1]][v[2]][v[3]];
    int zeros = n-(v[1]+v[2]+v[3]);
    ld div = (1.0-((ld)zeros/n));
    ld p = 1.0;
    if(v[1]){
        p+= ((ld)v[1]/n)*f({v[0],v[1]-1,v[2],v[3]});
    }
    if(v[2]){
        p+= ((ld)v[2]/n)*f({v[0],v[1]+1,v[2]-1,v[3]});
    }
    if(v[3]){
        p+=((ld)v[3]/n)*f({v[0],v[1],v[2]+1,v[3]-1});
    }
    return dp[v[1]][v[2]][v[3]] = (p/div);
}
...
void solve()
{
    ...
    dp.resize(n+1,vector<vector<ld>>(n+1,vector<ld>(n+1,-1.0)));
    cout<<f(v)<<endl;
}
