...
int dfs(int node, int currCats){
    if(t[node].size()==1 and currCats<=m and node!=1) return 1;
    if(currCats>m) return 0;
    visited[node] = true;
    int ans = 0;
    for(auto &x: t[node]){
        if(!visited[x]){
            if(!cats[x]) ans+=dfs(x,0);
            else ans+=dfs(x,currCats+1);
        }
    }
    return ans;
}
