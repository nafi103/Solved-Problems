...
vi v;
int n;
 int f(int i, int prev, int cnt, int prog){
    if(i==n) return cnt;
    int ans = INT_MAX;
    if(prog==INT_MAX or v[i]-prev==prog) ans = min(ans,f(i+1,v[i],cnt,(i==0? INT_MAX : v[i]-prev)));
    if(prog==INT_MAX or v[i]-prev+1==prog) ans = min(ans,f(i+1,v[i]+1,cnt+1,(i==0? INT_MAX : v[i]-prev+1)));
    if(prog==INT_MAX or v[i]-prev-1==prog) ans = min(ans,f(i+1,v[i]-1,cnt+1,(i==0? INT_MAX : v[i]-prev-1)));
    return ans;
}
 void solve()
