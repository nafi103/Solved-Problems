...
void makeBrunch(int node, int k, int remLevel){
    if(remLevel==0 or curr>n) return;
    for(int i = 1; i<k; i++){
        if(curr>n) return;
        int now = curr;
        t.pb({node,curr++});
        makeBrunch(now,k,remLevel-1);
    }
}
...
void solve()
{
    ...
    for(int i = 2; i<=d; i++){
        int canAdd = k-2,remLevel = min(i-1,d+1-i);
        while(canAdd--){
            int now = curr;
            t.pb({i,curr++});
            makeBrunch(now,k,remLevel-1);
