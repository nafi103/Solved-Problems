...
void solve()
{
    ...
    for(int i = 1; i<=n; i++){
        for(int j = 1; j<=k; j++){
            for(int x = 0; x<=j and i+x<=n; x++){
                int left = i+x,right = i-(j-x)-1;
                if(right<0) continue;
                int currLeftSum = pref[left] - pref[i], currRightSum = pref[i-1] - pref[right];
                int reducedLeft = currLeftSum - (x)*v[i], reducedRight = currRightSum - (i-1-right)*v[i];
                dp[left][j] = max(dp[left][j],reducedLeft+reducedRight);
            }
        }
        for(int j = k; j>=1; j--){
            for(int x = 1; x<j; x++){
                int prev = i-x-1;
                if(prev<0) continue;
                dp[i][j] = max(dp[i][j],dp[i][x]+dp[prev][j-x]);
            }
            debug(dp[i][j])
            ans = min(ans,pref[n] - dp[i][j]);
