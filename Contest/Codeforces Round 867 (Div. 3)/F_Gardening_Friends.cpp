...
int getRandomNumber(int l, int r) {return uniform_int_distribution<int>(l, r)(rng);} 
 /****************************************************************/
vector<vi>t;
vector<int>dist,rdist,maxx,smaxx,level;
 int findDistance(int node, int parent, int currLevel){
    if(dist[node]!=-1) return dist[node];
    level[node] = currLevel;
    int d = -1;
    for(auto &x: t[node]){
        if(x!=parent){
            d = max(d,findDistance(x,node,currLevel+1));
        }
    }
    return dist[node] = 1+d;
}
 void reroot(int node, int parent){
    ...
    if(parent!=-1){
        int another = maxx[parent]==dist[node]? smaxx[parent]+1:maxx[parent]+1;
        if(another>=mx){
            smx = mx;
             mx = another;
        }else if(another>smx){
    ...
    maxx[node] = mx;
    smaxx[node] = smx;
    rdist[node] = max(mx+1,smx+1);
    for(auto &x: t[node]){
        if(x!=parent) reroot(x,node);
    }
}
