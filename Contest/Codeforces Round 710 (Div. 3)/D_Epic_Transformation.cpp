...
void solve()
{
    ...
    while(d.size()>1){
        auto it = d.begin();
        auto it2 = d.find_by_order(1);
        int fVal = (*it).second, fCnt = (*it).first, secVal = (*it2).second, secCnt = (*it2).first;
        d.erase(d.begin());
        d.erase(d.begin());
        fCnt--;
        secCnt--;
        if(fCnt) d.insert({fCnt,fVal});
        if(secCnt) d.insert({secCnt,secVal});
    }
    if(d.empty()){
        cout<<0<<endl;
        return;
    }else{
        cout<<(*d.begin()).ff<<endl;
    }
}
