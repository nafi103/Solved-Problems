...
vector<int>dp;
 int ncr(int n, int r){
    if(r>n) return 0;
    return ((((fact[n])*(mminvprime(fact[r],mod)))%mod)*(mminvprime(fact[n-r],mod)))%mod;
}
 void solve()
{
    cin>>n;
    ...
    for(int i = n-2; i>=0; i--){
        dp[i] = dp[i+1];
        if(v[i]<=0) continue;
        if(n-1-i<v[i]) continue;
        dp[i]+= ncr(n-1-i,v[i]);
        dp[i]%=mod;
        int nextPoint = i+v[i]+1,prev = 0;
        while(nextPoint<n or dp[nextPoint]==0){
            dp[i]+=(ncr(nextPoint - i - 2, v[i]-1))*(dp[nextPoint])%mod;
            dp[i]%=mod;
            nextPoint++;
        }
        dp[i]%=mod;
