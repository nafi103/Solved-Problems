...
template <class T> using pbds = tree<T, null_type, greater_equal<T>, rb_tree_tag, tree_order_statistics_node_update >;
...
void solve()
{
    ...
    while(j<n){
        int color = 1;
        for(int p = i; p<=j; p++){
            ans[v[p].second] = color;
            color++;
        }
        int last = 1, p2 = j-1,next = 0;
        while(p2>=i and v[p2].ff==v[j].ff){
            last++;
            p2--;
        }
        p2 = j+1;
        while(p2<n and v[p2].ff==v[j].ff){
            next++;
            p2++;
        }
        i = j+1 + max(0ll,next-(k-last));
        j = i+k-1;
    }
    writev(ans);
}
