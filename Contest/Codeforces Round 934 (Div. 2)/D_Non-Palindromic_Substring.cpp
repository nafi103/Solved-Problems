...
bool palindromeFound(Hashing &original, Hashing &reversed, int l, int r, int n)
{
    int l2 = n - r - 1, r2 = n - l - 1;
    if (original.substringHash(l, r) == reversed.substringHash(l2, r2))
    {
        return true;
    }
    return false;
...
void solve()
{
    ...
    while(q--){
        ...
        vector<int> mark(4);
        for(int i = 0; i<4; i++){
            mark[i]=(palindromeFound(s,rs,l,r-i,n) and palindromeFound(s,rs,l+i,r,n));
        }
        debug(mark);
        ...
        int ans = f(r-l+1);
        if(mark[0]) ans-= (r-l+1);
        if((mark[0] and mark[2]) or mark[1] and mark[3]) ans = ap(r-l+1); 
        cout<<ans<<endl;
    }
}
