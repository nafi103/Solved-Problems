...
/****************************************************************/
vector<vi>t;
vi color,max_possible,final;

void find_max_using_child(int node, int parent){
    if(color[node]==0) max_possible[node] = -1;
    else max_possible[node] = 1;
    for(auto &x: t[node]){
        if(x!=parent){
            find_max_using_child(x,node);
            if(max_possible[x]>0) max_possible[node]+=max_possible[x];
        }
    }
}

void find_final(int node, int parent){
    if(node==1) final[node] = max_possible[node];
    else{
        if(max_possible[node]>=0){
            final[node] = max(final[parent],max_possible[node]);
        }else{
            final[node] = max_possible[node];
            if(final[parent]>0) final[node]+=final[parent];
        }
    }
    for(auto &x: t[node]){
        if(x!=parent) find_final(x,node);
    }
}
