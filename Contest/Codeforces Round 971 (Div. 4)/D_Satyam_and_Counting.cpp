...
void solve()
{
    ...
    for(auto &x: mp[0]){
        if(mp[1].find(x)!=mp[1].end()){
            ans+=(mp[0].size()+mp[1].size()-2);
        }
        if(mp[1].find(x-1)!=mp[1].end() and mp[1].find(x+1)!=mp[1].end()) ans++;
    }
    for(auto &x:mp[1]){
        if(mp[0].find(x-1)!=mp[0].end() and mp[0].find(x+1)!=mp[0].end()) ans++;
    }
    cout<<ans<<endl;
}
