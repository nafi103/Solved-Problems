...
int f(string str, int pos, int takenF, int takenS, int leftF, int leftS){
    if(takenF>takenS+leftS or takenF+leftF<takenS) return pos;
    if(pos==str.size()) return str.size();
    if(str[pos]=='?'){
        str[pos] = '0';
        int f1 = f(str,pos,takenF,takenS,leftF,leftS);
        str[pos] = '1';
        int f2 = f(str,pos,takenF,takenS,leftF,leftS);
        return min(f1,f2);
    }
    if(!(pos&1)) return f(str,pos+1,takenF+(str[pos]=='1'),takenS,leftF-1,leftS);
    return f(str,pos+1,takenF, takenS+(str[pos]=='1'),leftF,leftS-1);
}
