...
int getRandomNumber(int l, int r) {return uniform_int_distribution<int>(l, r)(rng);} 
 /****************************************************************/
string s,t;
 bool go(int tp, int sp,bool rightDone){
    if(tp == t.size()) return true;
    if(sp<0 or sp==s.size()) return false;
    if(s[sp]==t[tp]){
        if(rightDone) return go(tp+1, sp-1, true);
        return go(tp + 1, sp + 1, false) | go(tp + 1, sp - 1, true);
    }
    else return false;
}
 void solve()
