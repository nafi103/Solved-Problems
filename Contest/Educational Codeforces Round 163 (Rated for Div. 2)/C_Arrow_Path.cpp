...
void solve()
{
    ...
    vector<vector<bool>>mark(2,vector<bool>(n,false));
    if(arr[0][1]=='>') mark[0][2] = true;
    if(arr[1][0]=='>') mark[1][1] = true;
    mark[0][0] = true;
    for(int i = 3; i<n; i++){
        if(!(i&1)){if(arr[0][i-1]=='>'){
            if(mark[0][i-2] or mark[1][i-1]) mark[0][i] = true;
        }}
        else{if(arr[1][i-1]=='>'){
            if(mark[0][i-1] or mark[1][i-2]) mark[1][i] = true;
        }}
    }
    if(mark[0][n-1] or mark[1][n-1] or mark[1][n-2]){
        cout<<"YES"<<endl;
    }else{
        cout<<"NO"<<endl;
    }
