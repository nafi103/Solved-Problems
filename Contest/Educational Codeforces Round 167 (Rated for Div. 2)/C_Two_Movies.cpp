...
void solve()
{
    ...
    readv(a);
    readv(b);
    int ra = 0, rb = 0,commonNeg = 0,commonPos = 0;
    for(int i= 0; i<n; i++){
        if(a[i]==-1 and b[i]==-1){
            commonNeg++;
        }else if(a[i]==1 and b[i]==1) commonPos++;
        else if(a[i]==1) ra++;
        else if(b[i]==1) rb++;
    }
    if(ra>rb) swap(ra,rb);
    int need = rb-ra;
    ra+=min(need,commonPos);
    commonPos-=min(need,commonPos);
    if(commonPos){
        int tob = commonPos/2;
        int toa = commonPos - tob;
        ra+=toa;
        rb+=tob;
        commonPos = 0;
    }
    if(commonNeg){
        need = min(rb-ra,commonNeg);
        rb-=need;
        commonNeg-=need;
    }
    if(commonNeg){
        int ans = min(ra,rb), minus = (commonNeg+1)/2;
        cout<<ans-minus<<endl;
    }else{
        cout<<min(ra,rb)<<endl;
    }
