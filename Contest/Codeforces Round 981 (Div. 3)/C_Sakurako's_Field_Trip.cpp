...
void solve()
{
    ...
    readv(v);
    for(int i = 0; i<n/2; i++){
        int inv_at_i = (v[i]==v[i+1]);
        if(i) inv_at_i =  (v[i]==v[i-1]);
        int inv_at_ni = (v[n-i-1]==v[i-1]);
        if(n-i-1<n-1) inv_at_ni =  (v[n-i-1]==v[n-i]);
        swap(v[i],v[n-i-1]);
        int aft_inv_at_i = (v[i]==v[i+1]);
        if(i) aft_inv_at_i =  (v[i]==v[i-1]);
        int aft_inv_at_ni = (v[n-i-1]==v[i-1]);
        if(n-i-1<n-1) aft_inv_at_ni =  (v[n-i-1]==v[n-i]);
        if(inv_at_i+inv_at_ni<aft_inv_at_i+aft_inv_at_ni) swap(v[i],v[n-i-1]);
    }
    for(int i = 0; i<n-1; i++){
        if(v[i]==v[i+1]) ans++;
    }
