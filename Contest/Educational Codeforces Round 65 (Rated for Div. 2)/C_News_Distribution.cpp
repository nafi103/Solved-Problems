...
void dfs(int node, vector<bool> &visited,vector<int>&group){
    group.pb(node);
    visited[node] = true;
    for(auto &x: g[node]){
        if(!visited[x]) dfs(x,visited,group);
    }
}
...
void solve()
{
    ...
    for(int i = 0; i<m; i++){
        ...
        readv(people);
        for(int i = 1; i<k; i++){
            g[people[i]].pb(people[i-1]);
            g[people[i-1]].pb(people[i]);
        }
    }
