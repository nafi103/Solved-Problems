...
bool dfs(int i, int j,int parI, int parJ, char currChar){
    if(i==n or j==m or i<0 or j<0) return false;
    if(v[i][j]==currChar and visited[i][j]) return true;
    if(v[i][j]!=currChar) return false;
    visited[i][j] = true;
    bool flag = false;
    for(int k = 0; k<4; k++){
        int newI = i+dirx[k],newJ = j+diry[k];
        if(newI!=parI or newJ!=parJ) flag|= dfs(newI, newJ, i,j,currChar);
    }
    return flag;
}
...
void solve()
{
    ...
    for(int i = 0; i<n; i++){
        for(int j = 0; j<m; j++){
            if(!visited[i][j]) flag |= dfs(i,j,i,j,v[i][j]);
        }
    }
    if(flag) yes;
    else no;
