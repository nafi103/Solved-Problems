...
bool check(string str, int &k, int &n){
    int curr = str[0]-'0';
    for(int i = 0; i<n; i+=k){
        int currChar = '0'+curr;
        for(int j = i; j<i+k; j++){
...
int find(string &str, int k,int n){
    int needLast = 1,needChar = str[n-1]-'0';
    for(int i = n-2; str[i]==str[i+1]; i--){
        needLast++;
    }
    needLast= k-needLast;
    if(needLast==0){
    ...
    while(r<n){
        if(zCnt>k){
            if(needChar==0) return l+needLast-1;
            else return -1;
        }else if(oCnt>k){
            if(needChar==1) return l+needLast-1;
            else return -1;
        }
        r++;
        if(r==n) break;
...
string makeString(string str, int pos,int n){
    string newStr = "";
    for(int i = pos; i<n; i++){
        newStr.pb(str[i]);
    }
...
void solve()
{
    ...
    string str;
    cin>>str;
    int curr = str[0]-'0',pos = -1;
    for(int i = 0; i<n; i+=k){
        int currChar = '0'+curr;
        for(int j = i; j<i+k; j++){
            if(str[j]!=currChar) {pos = j;break;}
        }
        curr^=1;
        if(pos!=-1) break;
    }
    ...
    int moreThanK = find(str,k,n);
    if(check(makeString(str,pos+1,n),k,n)) cout<<pos+1<<endl;
    else if(check(makeString(str,pos,n),k,n)) cout<<pos<<endl;
    else if(check(makeString(str,moreThanK+1,n),k,n)) cout<<moreThanK+1<<endl;
    else cout<<-1<<endl;
}
