...
int f(int i, int last, int secondLast, int &first, int &second){
    if(i==n-1){
        int curr = INT_MAX;
        if((last!=0 or secondLast!=0) and (first!=0 or last!=0) and (first!=0 or second!=0)) curr = (v[i]!=0);
        if((last!=1 or secondLast!=1) and (first!=1 or last!=1) and (first!=1 or second!=1)) curr = min(curr,0ll+(v[i]!=1));
        return curr;
    }
    if(dp[i][last][secondLast]!=-1) return dp[i][last][secondLast];
    int curr = INT_MAX;
    if((last!=0 or secondLast!=0)) curr = (v[i]!=0) + f(i+1,0,last,first,second);
    if((last!=1 or secondLast!=1)) curr = min(curr, f(i+1,1,last,first,second)+(v[i]!=1));
    return dp[i][last][secondLast] = curr;
}
...
void solve()
{
    ...
    for(int i = 0; i<=1; i++){
        for(int j = 0; j<=1; j++){
            dp.resize(n,vector<vi>(2,vi(2,-1)));
            int curr = 0;
            if(j!=v[0]) curr++;
            if(i!=v[1]) curr++;
            ans = min(ans, curr+f(2,i,j,j,i));
            dp.clear();
        }
    }
    cout<<ans<<endl;
