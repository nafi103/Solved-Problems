...
void solve()
{
    ...
    int mx = 1,mxGovt = govts[0];
    for(int i = 0; i<k; i++){
        dfs(govts[i],num_of_nodes[i]);
        if(num_of_nodes[i]>mx){
            mx = num_of_nodes[i];
            mxGovt=govts[i];
        }
    }
    int diffNodes = 0,totalEdges = 0;
    for(int i = 1; i<=n; i++){
        if(!visited[i]) diffNodes++;
    }
    for(int i = 0; i<k; i++){
        if(govts[i]==mxGovt){
            int newNodes = num_of_nodes[i]+diffNodes;
            totalEdges+=(f(newNodes));
        }else{
            totalEdges+=(f(num_of_nodes[i]));
        }
    }
    cout<<totalEdges-m<<endl;
}
