...
pair<int, int> f(int start, string &str)
{
    int loss = 0, profit = 0;
    for (auto &x : str)
    {
...
void solve()
{
    ...
    for (int j = 0; j < n; j++)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            if (i == 0 or dp[(i + 4) % 5] != INT_MIN)
            {
                pii now = f(i, str[j]);
                int end = now.ss;
                if (i == 0)
                {
                    tmp[end] = max({tmp[end], now.ff, dp[4] + now.ff});
                }
                else
                {
                    tmp[end] = max(tmp[end], dp[(i + 4) % 5] + now.ff);
                }
            }
        }
        for (int i = 0; i < 5; i++)
    ...
    int ans = 0;
    for(int i = 0; i<5; i++){
        ans = max(ans,dp[i]-(((i+1)%5)*2));
    }
    cout << max(ans,dp[4]) << endl;
}
