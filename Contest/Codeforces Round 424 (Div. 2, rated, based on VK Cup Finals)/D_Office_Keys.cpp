...
int f(int person, int pos){
    if(person==n) return  0;
    if(pos>k) return inf;
    if(dp[person][pos]!=-1) return dp[person][pos];
    dp[person][pos] = max(abs(v[person] - mp[pos]) + abs(mp[pos]-p), f(person+1,pos+1));
    if(k-pos-1>=n-1-person) dp[person][pos] = min(dp[person][pos], f(person,pos+1));
    return dp[person][pos];
}
...
void solve()
{
    ...
    sort(all(v));
    sort(all(key));
    for(int i = 1; i<=k; i++) mp[i] = key[i-1];
    cout<<f(0,1)<<endl;
}
