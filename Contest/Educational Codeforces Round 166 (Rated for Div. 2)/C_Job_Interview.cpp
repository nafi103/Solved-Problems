...
void solve()
{
    ...
    vi programmer, tester;
    vector<int>total(n+m+1);
    for(int i = 0; i<n+m+1; i++){
        if((tester.size()==m+1) or (p[i]>t[i] and programmer.size()<=n)){
            programmer.pb(i);
            total[i] = p[i];
        }else{
            tester.pb(i);
    ...
    int lastTester = tester.back(),lastProgrammer = programmer.back();
    for(int i = 0; i<n+m+1; i++){
        int ans = 0;
        if(binary_search(all(programmer),i)){
            if(programmer.size()==n+1){
                if(i-1>=0) ans+=total[i-1];
                ans+=(total[n+m]-total[i]);
            }else{
                if(i-1>=0) ans+=total[i-1];
                ans+=(total[n+m]-total[i]);
                ans-=t[lastTester];
            ...
            if(tester.size()==m+1){
                ...
                if(i-1>=0) ans+=total[i-1];
                ans+=(total[n+m]-total[i]);
                ans-=p[lastProgrammer];
                ans+=t[lastProgrammer];
            }
        }
        cout<<ans<<" ";
    }
