...
int getRandomNumber(int l, int r) {return uniform_int_distribution<int>(l, r)(rng);} 
 /****************************************************************/
int f(int i, vector<bool> &visited, string &str, vi &v)
{
    if (visited[i])
        return 0;
    visited[i] = true;
    return (str[i - 1] == '0') + f(v[i], visited, str, v);
}
 void init(int i, vector<bool> &visited, vi &ans, int val, vi &v)
{
    if (visited[i])
        return;
    visited[i] = true;
    ans[i] = val;
    init(v[i], visited, ans, val, v);
}
void solve()
{
    int n;
    ...
    for(int i = 1; i<=n; i++){
        int val = f(i, visited, str, v);
        init(i, vis,ans, val, v);
    }
    for (int i = 1; i<=n; i++) {
        cout << ans[i] << " ";
    }
