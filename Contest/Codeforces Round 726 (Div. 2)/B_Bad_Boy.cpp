...
/****************************************************************/
pair<int,int> corners(int point, int n, int m){
    if(point==1) return {1,1};
    else if(point==2) return {1,m};
    else if(point==3) return {n,1};
    else return {n,m};
}

int distance(pii one, pii two){
    return abs(one.ff-two.ff)+ abs(one.ss-two.ss);
}

int total_distance(pii one, pii two, int i, int j){
    int d = 0;
    d+=distance({i,j},{one});
    d+=distance(one,two);
    d+=distance(two,{i,j});
...
void solve()
{
    ...
    int n,m,I,J,mx = INT_MIN;
    cin>>n>>m>>I>>J;
    vi mark = {1,2,3,4};
    for(int i = 0; i<4; i++){
        for(int j = i; j<4; j++){
            int d = total_distance(corners(mark[i],n,m),corners(mark[j],n,m),I,J);
            if(d>mx){
                mx = d;
                ans1 = corners(mark[i],n,m);
                ans2 = corners(mark[j],n,m);
            }
        }
    }
    cout<<ans1.ff<<" "<<ans1.ss<<" "<<ans2.ff<<" "<<ans2.ss<<endl;
}
