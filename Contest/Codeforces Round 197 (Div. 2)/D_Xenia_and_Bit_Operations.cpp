...
int getRandomNumber(int l, int r) {return uniform_int_distribution<int>(l, r)(rng);} 
 /****************************************************************/
vi v, t;
 int buildSegTree(int node, int b, int e){
    if(b==e){
        t[node] = v[e];
        return 0;
    }
    int left = 2*node, right = 2*node+1, mid = (b+e)/2;
    int leafLevel = 1+buildSegTree(left, b, mid);
    buildSegTree(right, mid+1, e);
    if(leafLevel&1) t[node] = t[left]|t[right];
 ...
 int update(int node, int b,int e, int i, int x){
    ...
    int left = 2*node, right = 2*node+1, mid = (b+e)/2,leaf;
    if(i<=mid) leaf = 1+update(left, b,mid, i, x);
    else leaf = 1+update(right, mid+1, e, i, x);
    if(leaf&1) t[node] = t[left]|t[right];
    else t[node] = t[left]^t[right];
    return leaf;
}
 void solve()
