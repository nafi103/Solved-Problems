...
int getRandomNumber(int l, int r) {return uniform_int_distribution<int>(l, r)(rng);} 
 /****************************************************************/
 int findPos(vector<vi>&v, int sum, int i){
    int currSum = accumulate(all(v[i]),0ll);
    for(int j = 0; j<v[i].size(); j++){
        if(currSum-v[i][j]==sum) return j;
    }
    return -1;
}
 void solve()
{
    ...
    for(int i = 0; i<n; i++){
        ...
        readv(v[i]);
        set<int>s;
        int sum = accumulate(all(v[i]),0ll);
        for(auto &x: v[i]) s.insert(sum-x);
        for(auto &x: s) mp[x].pb(i);
    }
    bool flag = false;
    ...
    ans[0][1] = findPos(v,sum,ans[0][0]);
    ans[1][1] = findPos(v,sum,ans[1][0]);
    yes;
    for(auto &x: ans){
