...
void sendLast(string &tmp, int p, int finalPos, int &ans){
    if(p==0 and tmp[1]=='0'){
        int p = 2;
        while(p<n and str[p]=='0') p++;
        if(p==n){
            ans = INT_MAX;
            return;
        }
...
int f(char sl, char fl){
    int fp=n-1,ans = 0,sp = n-1;
    string tmp = str;
    while(fp>=0 and str[fp]!=fl) fp--;
    while(sp>=0 and (str[sp]!=sl or sp==fp)) sp--;
    if(sp<0 or fp<0) return INT_MAX;
    if(sp<fp){
        sendLast(tmp,fp,n-1,ans);
        sendLast(tmp,sp,n-2,ans);
    }else{
        sendLast(tmp,sp,n-1,ans);
        sendLast(tmp,fp,n-1,ans);
    }
    return ans;
}
