...
void solve()
{
    ...
    for(auto &x: s){
        for(int i = 1; i<INT_MAX; i<<=1){
            int prev = x-i,next = x+i;
            bool prevFound = (s.find(prev)!=s.end()), nextFound = (s.find(next)!=s.end());
            if(prevFound and nextFound){
                cout<<3<<endl;
                cout<<x<<" "<<prev<<" "<<next<<endl;
                return;
            }
            if(prevFound or nextFound){
                two = true;
                t1 = x;
                t2 = (prevFound? prev:next);
            }
        }
    }
    if(two){
