...
void solve()
{
    int n;
    cin>>n;
    vi a(n+1),t(n+1), pref(n+1,0),extra(n+1,0),diff(n+1,0);
    for(int i =1 ; i<=n; i++) cin>>a[i];
    for(int i = 1 ; i<=n; i++) cin>>t[i];
    for(int i = 1; i<=n; i++) pref[i] = pref[i-1]+t[i];
    for(int i = 1; i<=n; i++){
        debug(diff)
        int find = pref[i-1]+a[i];
        debug(find)
        int pos = upper_bound(all(pref),find) - pref.begin();
        debug(pos)
        diff[i]++;
        if(pos==n+1) continue;
        diff[pos]--;
        int minus = pref[pos-1]-pref[i-1];
        extra[pos] += a[i]-minus;
    }
    debug(extra)
