...
bool same(string &a, string &b){
    return a[0]==b[0] or a[0]==b[1] or b[0]==a[1] or b[1]==a[1]; 
}
...
void solve()
{
    ...
    while(q--){
        ...
        int lessX = INT_MIN, greaterX = INT_MAX;
        if(x>y) swap(x,y);
        for(auto &[str,s]: mp){
            if(str==v[x] or str==v[y]){
                continue;
            }
            auto findLessX = lower_bound(all(s),x);
            if(findLessX!=s.begin()) findLessX--;
            if(*findLessX<x) lessX = max(lessX,*findLessX);
            auto findGreaterX = upper_bound(all(s),x);
            if(findGreaterX!=s.end()) greaterX = min(*findGreaterX,greaterX);
            if(greaterX<y){
                break;
            }
        }
        ...
        if(lessX==INT_MIN and greaterX==INT_MAX){
            cout<<-1<<endl;
        }
        else cout<<y-x+2*min(x-lessX,greaterX-y)<<endl;
    }
}
