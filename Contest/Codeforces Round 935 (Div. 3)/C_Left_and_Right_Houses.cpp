...
void solve()
{
    ...
    for(int i = 0; i<n; i++){
        if(str[i]=='0'){
            pref[i+1] = {pref[i].first+1,pref[i].second};
        }else{
            pref[i+1] = {pref[i].first,pref[i].second+1};
        }
    }
    vector<int>possibleAns;
    for(int i = 0; i<=n; i++){
        if(pref[i].ff>=pref[i].ss and pref[n].ss-pref[i].ss>=pref[n].ff-pref[i].ff) possibleAns.pb(i);
    }
    int ans = *possibleAns.begin();
    double val = ((double)n/2-ans);
    for(int i = 1; i<possibleAns.size(); i++){
        if(abs((double)n/2 - possibleAns[i]) < val){
            val = abs((double)n/2 - possibleAns[i]);
            ans = possibleAns[i];
        }
    }
    cout<<ans<<'\n';
