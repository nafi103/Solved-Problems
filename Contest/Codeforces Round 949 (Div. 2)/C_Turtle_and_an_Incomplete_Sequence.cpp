...
void solve()
{
    ...
    if(store.size()){
        int cnt = 1,num = store[0].ss;
        if(num==1) cnt++;
        for(int i = store[0].ff - 1; i>=0; i--){
            if(cnt&1){
                num/=2;
                ans[i] = num;
            }else{
    ...
    int sz = store.size(),l = 0,r = 1;
    while(r<sz){
        if(store[r].ff-store[l].ff==1){
            if(store[r].ss/2!=store[l].ss and store[r].ss!=store[l].ss/2){
                cout<<-1<<endl;
                return;
            }
            l++;
        ...
        int lp = store[l].ff, rp = store[r].ff,ln = store[l].ss,rn = store[r].ss;
        if(ln<rn and rn>1){
            rp--;
            rn/=2;
            ans[rp] = rn;
    ...
    int last = (store.back()).ff,num = (store.back()).ss;
    int cnt = 1;
    if(num==1) cnt++;
