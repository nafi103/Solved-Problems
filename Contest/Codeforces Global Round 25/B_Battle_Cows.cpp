...
int swapWithMax(int n,int k,vi v){
    int i,cnt = 0;
    for(i = 0; i<n; i++){
        if(i>=k-1 or v[i]>v[k-1]) break;
    }
    if(i==k-1) return INT_MIN;
    swap(v[k-1],v[i]);
    if(i!=0) cnt++;
    k = i;
    for(++i; i<n; i++){
...
int swapWith0(int n, int k, vi v){
    swap(v[k-1],v[0]);
    int cnt = 0;
    for(int i = 1; i<n; i++){
        if (v[i] < v[0])
...
void solve()
{
    ...
    vi v(n);
    readv(v);
    cout<<max(swapWithMax(n,k,v),swapWith0(n,k,v))<<endl;
}
