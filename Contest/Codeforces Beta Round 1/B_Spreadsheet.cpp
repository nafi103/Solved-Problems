...
int getRandomNumber(int l, int r) {return uniform_int_distribution<int>(l, r)(rng);} 
 /****************************************************************/
string convertOne(string column, string row){
    int colNum = 0,power = 0;
    while(column.size()){
        colNum+=(pow(26,power)*(column.back()-'A'+1));
        power++;
        column.pop_back();
    }
    return 'R'+row+'C'+to_string(colNum);
}
 string convertTwo(string row, string column){
    string colPart = "";
    int n = stoll(column);
    while(n>0){
        int rem = (n - 1)%26;
 ...
 void solve()
{
    ...
    while(str.back()>='A' and str.back()<='Z'){
        partOne.push_back(str.back());
        str.pop_back();
    }
    while(str.size() and str.back()>='0' and str.back()<='9'){
        partTwo.push_back(str.back());
        str.pop_back();
    }
    if(str.empty()){
        cout<<convertOne(partOne,partTwo)<<endl;
    }else{
        reverse(all(str));
        cout<<convertTwo(partTwo,str.substr(1))<<endl;
    }
}
 int32_t main()
{
