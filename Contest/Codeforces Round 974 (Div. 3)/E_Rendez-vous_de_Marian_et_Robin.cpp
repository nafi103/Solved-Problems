...
vector<vi> dijsktra(int src)
{
    priority_queue<array<int, 3>, vector<array<int, 3>>, greater<array<int, 3>>> pq;
    vector<vi> distance(3, vi(n + 10, inf));
    distance[0][src] = 0;
    if (horse[src])
        distance[1][src] = 0;
    ...
    while (!pq.empty())
    {
        ...
        pq.pop();
        if (distance[p[2]][p[1]] < p[0]) {
            continue;
        }
        if (p[2])
        {
            for (auto &[nbr, wt] : v[p[1]])
            {
                if (distance[1][nbr] > p[0] + wt / 2)
                {
                    distance[1][nbr] = p[0] + wt / 2;
                    pq.push({distance[1][nbr], nbr, 1});
                }
            }
        }
        else
        {
            for(auto &[nbr,wt]:v[p[1]]){
                if(horse[nbr]){
                    if(distance[1][nbr]>p[0]+wt){
                        distance[1][nbr] = p[0] + wt;
                        pq.push({distance[1][nbr], nbr, 1});
                    }
                }else{
                    if(distance[0][nbr]>p[0]+wt){
                        distance[0][nbr] = p[0]+wt;
                        pq.push({distance[0][nbr], nbr, 0});
                    }
                }
            }
        }
 ...
 void solve()
{
    ...
    vector<vi>dist1 = dijsktra(1),dist2 = dijsktra(n);
    int ans = inf;
    for(int i = 1; i<=n; i++){
        ans = min(ans,max(min(dist1[0][i],dist1[1][i]),min(dist2[0][i],dist2[1][i])));
    }
