...
vector<int> value;
vector<vector<int>>t;
 bool dfs(int node, int parent, int need){
    if(value[node]<need){
        if(t[node].size()==1 or need>1e9) return false;
        need = 2*need - value[node];
    }
    bool flag = true;
    for(auto &x: t[node]){
        if(x!=parent){
            flag&=dfs(x,node,need);
        }
    }
    return flag;
}
 int check(int need){
    if(need<=value[1]) return true;
    bool flag = true;
    for(auto &x: t[1]){
        flag&=dfs(x,1,need - value[1]);
    }
    return flag;
}
 int bs(int l, int r){
    if(l>r) return r;
    int mid = (l+r)/2;
    if(check(mid)) return bs(mid+1,r);
 ...
 void solve()
{
    ...
    int ans = bs(value[1],value[1]+*max_element(all(value)));
    cout<<ans<<endl;
    value.clear();
    t.clear();
}
