...
void solve()
{
    ...
    int prev = pref[0], blast = 0,alast = pref[0];
    int l = 0,r = n-1;
    bool bob = true;
    while(r-l>1){
        if(bob){
            r = upper_bound(suff.rbegin(),suff.rend(),prev+blast) - suff.rbegin();
            r = max(0ll,n-1-r);
            prev = suff[r]-blast;
            blast = suff[r];
            bob = false;
        }else{
            l = upper_bound(all(pref),prev+alast) - pref.begin();
            l = min(l,n-1);
            prev = pref[l] - alast;
            alast = pref[l];
            bob = true;
