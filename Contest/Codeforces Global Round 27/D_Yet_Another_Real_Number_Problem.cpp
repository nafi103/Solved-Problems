...
vi power_of_two(MAXN*29+1);

bool cmp(int prev, int curr, int currP){
    if(currP>29) return true;
    return prev<(curr*power_of_two[currP]);
}
...
void solve()
{
    ...
    for(int i = 0; i<n; i++){
        ...
        x>>=cnt;
        while(!st.empty() and cmp(st.top().ss,x,cnt)){
            auto [p,val] = st.top();
            st.pop();
            ans=(ans+val-power_of_two[p]*val+MAXN*mod)%mod;
            cnt+=p;
        }
        ans=(ans+x*power_of_two[cnt])%mod;
        cout<<ans<<" ";
        if(cnt) st.push({cnt,x});
    }
    cout<<endl;
}
