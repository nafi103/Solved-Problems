...
/****************************************************************/
vector<vector<pii>>t;
vector<vi>mini,parents;
vector<int>level;
int mx;

void dfs(int node,int parent, int weight, int clevel){
    parents[0][node] = parent;
    level[node] = clevel;
    mini[0][node] = weight;
    for(auto &[x,w]: t[node]){
        if(x!=parent) dfs(x,node,w,clevel+1);
    }
}
...
int minDistance(int a, int b){
    int LCA = lca(a,b);
    int distanceFroma = level[a] - level[LCA],distanceFromB = level[b]-level[LCA];
    int mnFrA = minimumTillK(a,distanceFroma);
    int mnFrB = minimumTillK(b,distanceFromB);
    return min(mnFrA,mnFrB);
}
    ...
    for(int i = 1; i<mx; i++){
        for(int j = 1; j<=n; j++){
            ...
            if(prevParent!=-1) {
                ...
                if(mini[i-1][prevParent]!=INT_MAX) mini[i][j] = min(mini[i-1][j],mini[i-1][prevParent]);
            }
        }
    }
    debug(mini);
