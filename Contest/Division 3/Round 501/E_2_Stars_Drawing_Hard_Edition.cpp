...
bool star(int i, int j){
    return i>=1 and i<=n and j>=1 and j<=m and grid[i][j]=='*';
}

bool checkMark(int i, int j,int k){
    if(i+k<=n and i-k>=1 and j+k<=m and j-k>=1){
        return mark[i+k][j] and mark[i-k][j] and mark[i][j+k] and mark[i][j-k];
    }
    return false;
}

void markTrue(int i, int j, int k){
    mark[i+k][j] = true;
    mark[i-k][j] = true;
    mark[i][j+k] = true;
    mark[i][j-k] = true;
...
void check(int i, int j){
    ...
    for(int k = 1; k<=min(n,m);k++){
        if(checkMark(i,j,k)) continue;
        if(star(i+k,j) and star(i-k,j) and star(i,j+k) and star(i,j-k)){
            mark[i][j]= true;
            markTrue(i,j,k);
        }else{
            cnt = k-1;
...
void solve()
{
    ...
    mark.assign(n+1,vector<bool>(m+1,false));
    for(int i = 1; i<=n; i++){
