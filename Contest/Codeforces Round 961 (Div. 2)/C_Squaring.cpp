...
int getRandomNumber(int l, int r) {return uniform_int_distribution<int>(l, r)(rng);} 
 /****************************************************************/
const ld eps = .0000000001;
 int check(int mid, ld &prev, ld &now){
    return prev-(mid*log(2)+now)<=eps;
}
 int bs(int l, int r,ld &prev, ld &now){
    int mid = (l+r)/2;
    if(l>r) return l;
    if(check(mid,prev,now)) return bs(l,mid-1,prev,now);
    else return bs(mid+1,r,prev,now);
}
 void solve()
{
    debug(log(2))
    ...
    int ans=0;
    for(int i = 1; i<n; i++){
        if(v[i]==log(log(1)) and v[i-1]!=log(log(1))){
            cout<<-1<<endl;
            return;
        }
        if(v[i]>=v[i-1]) continue;
        int newPow = bs(1,1e15,v[i-1],v[i]);
        debug(newPow);
        ans+=newPow;
        v[i]+= newPow*log(2);
    }
