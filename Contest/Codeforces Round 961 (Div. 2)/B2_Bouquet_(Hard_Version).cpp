  ...
  void solve()
{
    ...
    for(int i = 0; i<n; i++){
        if(v[i].ff>k) break;
        int canTake = min(v[i].ss,k/v[i].ff) * v[i].ff;
        if(i>0 and v[i].ff-v[i-1].ff==1){
            prev = min(v[i-1].ss, k / v[i-1].ff) * v[i-1].ff;
            int diff = k - prev;
            int newCanTake = prev + (min(diff / v[i].ff, v[i].ss) * v[i].ff);
            int remI = v[i].ss - min(diff / v[i].ff, v[i].ss);
            int newDiff = k - newCanTake;
            newCanTake += min({remI, newDiff, min(v[i - 1].ss, k / v[i-1].ff) });
            ans = max(ans, newCanTake);
        }
        ans= max(ans,canTake);
    }
    cout<<min(ans,k)<<endl;
