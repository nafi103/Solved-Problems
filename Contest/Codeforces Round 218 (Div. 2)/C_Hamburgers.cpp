...
int getRandomNumber(int l, int r) {return uniform_int_distribution<int>(l, r)(rng);} 
 /****************************************************************/
int cb,cs,cc,nb,ns,nc,pb,ps,pc,money;
 bool check(int burgers){
    int needB = burgers*cb, needS = burgers*cs, needC = burgers*cc;
    int buyB = max(0ll,needB-nb), buyS = max(0ll,needS-ns), buyC = max(0ll,needC-nc);
    int cost = (buyB*pb+buyS*ps+buyC*pc);
    return cost<=money;
 }
 int bs(int l, int r){
    if(l>r) return r;
